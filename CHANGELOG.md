...
- Promoted Pull Lab to its feature-owned single presentation runtime after zero-mismatch browser parity. The historical `applyPullLab` writer is physically retired from `wcl-runtime.js`; legacy orchestration delegates exactly once through `window.applyPullLabSource?.()`. `apps/web/src/features/pull-lab/runtime.js` and `public/pull-lab-runtime.js` remain byte-identical, add zero direct requests/timers/observers, and Browser Data Truth now validates the actual `single-source-owner` rather than a migration shadow. The owner checkpoint passed full `AvoiD Validation` run `32248756026`, including all nine protected screens and the production build.
