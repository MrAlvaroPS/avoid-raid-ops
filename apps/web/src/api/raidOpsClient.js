...
function activity(message, state = "busy", detail = {}) {
  if (typeof window === "undefined") return;
  window.dispatchEvent(new CustomEvent("avoid:activity", { detail: { at: Date.now(), message, state, ...detail } }));
}
...
async function parsePayload(response) {
  ...
  catch { throw new RaidOpsApiError("Service returned invalid JSON", { status: response.status, code: "INVALID_JSON", service: serviceName(response.url) }); }
}
...
async function requestJson(url, { method = "GET", body, timeoutMs = DEFAULT_TIMEOUT_MS, retries = method === "GET" ? 1 : 0 } = {}) {
  ...
  while (true) {
    ...
    activity(`${method} ${service}`, "busy", { service, attempt });
    try {
      ...
      if (!response.ok || payload?.ok === false) {
        ...
        throw new RaidOpsApiError(payload?.error || `HTTP ${response.status}`, {
      ...
      activity(`${service} ready`, "ready", { service, status: response.status });
      return payload;
    } catch (error) {
      const timeout = controller.signal.aborted;
      const normalized = error instanceof RaidOpsApiError ? error : new RaidOpsApiError(
        timeout ? `Request timed out after ${timeoutMs}ms` : (error?.message || String(error)),
        { code: timeout ? "TIMEOUT" : "NETWORK_ERROR", service, retryable: true, cause: error },
      );
      if (attempt < retries && normalized.retryable) {
        attempt += 1;
        activity(`${service} retry ${attempt}/${retries}`, "busy", { service, code: normalized.code });
        await wait(RETRY_DELAY_MS * attempt);
        continue;
      }
      activity(`${service} failed · ${normalized.message}`, "error", { service, code: normalized.code, status: normalized.status });
      throw normalized;
    } finally {
      clearTimeout(timer);
    }
