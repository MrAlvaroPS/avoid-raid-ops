...
test('CRITICAL v3.9.10 PORTABILITY: generic learning contract forbids boss-specific prerequisites and requires automatic portability tests',async()=>{
  const [agents,pipeline,holdout,discovery,pool,acquisition]=await Promise.all([
    ...
    readFile('server/corpus/untouched-holdout-acquisition-v1.mjs','utf8'),
  ]);
  ...
  assert.match(pipeline,/Untouched Holdout\*\* `\[implemented: automatic source discovery \+ precommit \+ combat acquisition \+ evaluation\]`/i);
  assert.match(holdout,/generic stage for \*\*any\*\* GLOBAL BOSS scope/i);
  assert.match(holdout,/operator does not hand-author a source list/i);
  assert.match(holdout,/caller-authored `holdoutEvidence`/i);
