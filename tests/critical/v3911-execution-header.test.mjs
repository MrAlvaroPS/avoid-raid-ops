...
test('CRITICAL v3.9.11 FIRST LOAD: HOME history reader is physically isolated from WCL network code',async()=>{
  ...
  assert.match(index,/avoid-execution-context-v3911\.js\?v=3\.9\.11\.0/);assert.match(index,/WCL stays idle until you explicitly refresh history or load a report/);
});
...
test('CRITICAL v3.9.11 HEADER: pull selection, explicit history refresh and active live report are independent controls',async()=>{
  ...
  assert.match(runtime,/activeReportDoesNotMutateHomeHistory:true/);assert.match(runtime,/pullSelectionIsConsumerOptIn:true/);assert.match(runtime,/firstPageWclNetworkAllowed:false/);
