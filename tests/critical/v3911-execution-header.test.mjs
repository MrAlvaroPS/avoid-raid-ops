...
test('CRITICAL v3.9.11 FIRST LOAD: HOME history reader is physically isolated from WCL network code',async()=>{
  ...
  assert.match(index,/avoid-execution-context-v3911\.js\?v=3\.9\.12\.0/);assert.match(index,/WCL stays idle until you explicitly refresh history or load a report/);
});

test('CRITICAL v3.9.11+ HEADER: log/pull selection, explicit history refresh and active live report are independent controls',async()=>{
  ...
  assert.match(runtime,/AVOID HISTORY/);assert.match(runtime,/ACTIVE WCL/);assert.match(runtime,/data-exec-reports/);assert.match(runtime,/data-exec-pulls/);assert.match(runtime,/data-exec-refresh/);assert.match(runtime,/data-exec-live/);assert.match(runtime,/data-exec-launch/);assert.match(runtime,/data-exec-stop/);
  ...
  assert.match(runtime,/activeReportDoesNotMutateHomeHistory:true/);assert.match(runtime,/historyReportSelectionIsConsumerOptIn:true/);assert.match(runtime,/pullSelectionIsConsumerOptIn:true/);assert.match(runtime,/firstPageWclNetworkAllowed:false/);assert.match(runtime,/activeReportNeverFetchesHomeHistory:true/);
  ...
  assert.match(runtime,/function renderSidebarHistory/);assert.match(runtime,/pulls stored/);assert.match(runtime,/button\)button\.hidden=true/);assert.match(runtime,/APP_RELEASE='3\.9\.12'/);
...
test('CRITICAL v3.9.11+ ACTIVE DATA: manifest controls rich operational hydration and Active Report never calls HOME history',async()=>{
  const runtime=await read('public/avoid-execution-context-v3911.js');
  assert.match(runtime,/function activeHydrationKeys/);assert.match(runtime,/lastReportHydrationKey/);assert.match(runtime,/lastOperationalHydrationKey/);
  assert.match(runtime,/new URL\(['"]\/api\/wcl\/report['"]/);assert.match(runtime,/new URL\(['"]\/api\/wcl\/operational-execution['"]/);
  assert.doesNotMatch(runtime,/new URL\(['"]\/api\/wcl\/history['"]/);
  assert.match(runtime,/if\(keys\.operationalKey/);assert.match(runtime,/completed-pull only/);
