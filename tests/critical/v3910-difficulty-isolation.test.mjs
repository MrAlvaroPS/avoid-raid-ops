...
test('CRITICAL v3.9.10 DIFFICULTY: fight selection never mixes the same boss across difficulties',()=>{
  const fights=[
    {id:1,encounterID:7777,difficulty:3,startTime:100,endTime:200},
    {id:2,encounterID:7777,difficulty:4,startTime:300,endTime:400},
    {id:3,encounterID:7777,difficulty:5,startTime:500,endTime:600},
    {id:4,encounterID:7777,difficulty:4,startTime:700,endTime:800},
  ];
...
test('CRITICAL v3.9.10 DIFFICULTY: report telemetry history intelligence and live status all propagate explicit difficulty',async()=>{
  const files=await Promise.all([
    read('server/services/report-service.mjs'),read('server/engines/report-engine.mjs'),
    read('server/services/telemetry-service.mjs'),read('server/engines/telemetry-engine.mjs'),
    read('server/services/history-service.mjs'),read('server/engines/history-engine.mjs'),
    read('server/services/intelligence-service.mjs'),read('server/engines/intelligence-engine.mjs'),
    read('server/services/live-status-service.mjs'),read('server/engines/status-engine.mjs'),
  ]);
...
test('CRITICAL v3.9.10 DIFFICULTY: official bootstrap uses build-pinned Journal difficulty metadata and never promotes it to combat truth',async()=>{
  const [provider,compiler,bootstrap,route,catalog]=await Promise.all([
    read('server/knowledge/providers/wago-db2-journal-difficulty-v1.mjs'),
    read('server/knowledge/official-encounter-difficulty-v1.mjs'),
    read('server/knowledge/raid-official-bootstrap-v1.mjs'),
    read('routes/api/wcl/mechanic-knowledge.js'),
    read('server/knowledge/raid-catalog-v1.mjs'),
  ]);
