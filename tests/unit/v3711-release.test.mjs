...
test('v3.9.3 keeps browser component versions honest while advancing package and Iris capability contract',async()=>{
  ...
  assert.match(pkg,/"version": "0\.3\.9-3-vercel\.0"/);assert.match(pkg,/"iris": "node --env-file=\.env\.local scripts\/iris-local-worker\.mjs"/);
  assert.match(iris,/const RELEASE='3\.8\.5'/);assert.match(iris,/const IRIS='Iris'/);assert.match(iris,/const RAID_LEADER='Onie'/);assert.match(players,/const VERSION='3\.9\.2'/);assert.match(capabilities,/release:'3\.9\.3'/);
