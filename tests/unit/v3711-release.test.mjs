...
test('v3.8.16 package metadata is active while the v3.8.8 Players hotfix and shared runtimes remain preserved',async()=>{
  ...
  assert.match(pkg,/"version": "0\.3\.8-16-vercel\.0"/);assert.match(pkg,/"iris": "node --env-file=\.env\.local scripts\/iris-local-worker\.mjs"/);
