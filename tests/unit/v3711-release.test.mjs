...
test('index preserves historical styles and adds v3.8.6 player intelligence after the shared data runtimes',async()=>{
  ...
  assert.match(index,/wcl-runtime\.js\?v=3\.8\.5/);assert.match(index,/progress-runtime-v3713\.js\?v=3\.8\.5/);assert.match(index,/iris-runtime-v3713\.js\?v=3\.8\.5/);assert.match(index,/player-intelligence-v386\.js\?v=3\.8\.6/);
  ...
  assert.ok(index.indexOf('/wcl-runtime.js?v=3.8.5')<index.indexOf('/player-intelligence-v386.js?v=3.8.6'));
  assert.ok(index.indexOf('/iris-runtime-v3713.js?v=3.8.5')<index.indexOf('/player-intelligence-v386.js?v=3.8.6'));
});
...
test('v3.8.7 package metadata is active while v3.8.5 shared runtimes and v3.8.6 player intelligence remain preserved',async()=>{
  ...
  assert.match(pkg,/"version": "0\.3\.8-7-vercel\.0"/);assert.match(pkg,/"iris": "node --env-file=\.env\.local scripts\/iris-local-worker\.mjs"/);
  assert.match(iris,/const RELEASE='3\.8\.5'/);assert.match(iris,/const IRIS='Iris'/);assert.match(iris,/const RAID_LEADER='Onie'/);
  assert.match(players,/const VERSION='3\.8\.6'/);
});