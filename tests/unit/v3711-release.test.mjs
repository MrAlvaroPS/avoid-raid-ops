...
test('index preserves historical styles and activates the v3.9.1 Players layer after the v3.9.0 data platform',async()=>{
  const index=await read('index.html');
  assert.match(index,/raidops-v3711\.css\?v=3\.7\.11/);assert.match(index,/raidops-v3712\.css\?v=3\.7\.12/);assert.match(index,/raidops-v3713\.css\?v=3\.8\.5/);assert.match(index,/raidops-v386\.css\?v=3\.8\.6/);assert.match(index,/raidops-v390\.css\?v=3\.9\.0/);assert.match(index,/raidops-v391\.css\?v=3\.9\.1/);
  assert.match(index,/wcl-bootstrap-v389\.js\?v=3\.8\.9\.1/);assert.match(index,/data-hub-v390\.js\?v=3\.9\.0/);assert.match(index,/knowledge-reindex-v390\.js\?v=3\.9\.0/);assert.match(index,/wcl-runtime\.js\?v=3\.8\.5/);assert.match(index,/progress-runtime-v3713\.js\?v=3\.8\.5/);assert.match(index,/iris-runtime-v3713\.js\?v=3\.8\.9\.1/);assert.match(index,/player-intelligence-v391\.js\?v=3\.9\.1/);
  ...
  assert.ok(index.indexOf('/wcl-runtime.js?v=3.8.5')<index.indexOf('/player-intelligence-v391.js?v=3.9.1'));
  assert.ok(index.indexOf('/iris-runtime-v3713.js?v=3.8.9.1')<index.indexOf('/player-intelligence-v391.js?v=3.9.1'));
});
...
test('v3.9.1 keeps one visible release-label owner and Players cannot fight it',async()=>{
  const [pkg,iris,players,bootstrap]=await Promise.all([read('package.json'),read('public/iris-runtime-v3713.js'),read('public/player-intelligence-v391.js'),read('public/wcl-bootstrap-v389.js')]);
