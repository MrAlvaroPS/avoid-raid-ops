...
test('matched null planner balances sources and keeps controls in the same fight/phase outside the episode exclusion',()=>{
  ...
  assert.equal(plan.plannedControls,6);assert.equal(plan.plannedSources,6);assert.equal(plan.sufficientByPlan,true);assert.equal(plan.evidenceContract.localFlankControlsUsed,false);
  for(const control of plan.controls){assert.equal(control.match.sameFight,true);assert.equal(control.match.sameOutcome,true);assert.equal(control.match.phaseAvailable,true);assert.equal(control.match.phaseMatched,true);assert.ok(control.match.temporalDistanceMs>control.match.episodeExclusionDistanceMs);assert.ok(control.windowStart>=0&&control.windowEnd<=120000);}
});
...
test('matched null preview is network-free and exposes bounded exact-fight work',()=>{
  const {evidence,profiles}=syntheticPlanInputs();
  const plan=buildMatchedNullBaselinePlanV1({episode,evidenceRecords:evidence,profiles,config:{maxControls:6,maxControlsPerSource:1}}),preview=buildMatchedNullBaselinePreviewV1({plan,maxWclCalls:12,maxContinuationRounds:1});
  assert.equal(preview.executesWcl,false);assert.equal(preview.wclCallsExecuted,0);assert.equal(preview.controlsRemaining,6);assert.equal(preview.networkUpperBound.preflightCalls,1);assert.equal(preview.networkUpperBound.initialControlCalls,6);assert.equal(preview.executionPolicy.exactFightIDsOnly,true);assert.equal(preview.executionPolicy.wholeReportFallback,false);assert.equal(preview.executionPolicy.localFlankBaselineIsPromotionBaseline,false);
});
