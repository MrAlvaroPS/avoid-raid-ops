...
test('active compatibility asset manifest makes the current production stack explicit',()=>{
  ...
  const pullLabSource=ACTIVE_LOCAL_SCRIPTS.find(asset=>asset.id==='pull-lab-source-runtime');
  assert.equal(pullLabSource?.src,'/pull-lab-runtime.js?v=4.0.0-migration7-owner1');
  assert.equal(pullLabSource?.owner,'pull-lab-source');
  assert.equal(pullLabSource?.sourceOwner,'apps/web/src/features/pull-lab/runtime.js');
  assert.equal(pullLabSource?.authority,'source-owner');
  assert.equal(pullLabSource?.role,'single-source-pull-lab-presentation');
  assert.equal(pullLabSource?.retirement,'keep-stable-source-owned-transport');
...
test('runtime domains have one primary owner while the monolithic WCL runtime is compatibility-only',()=>{
  ...
  assert.ok(ACTIVE_LOCAL_SCRIPTS.indexOf(legacy)<ACTIVE_LOCAL_SCRIPTS.indexOf(pullLabSource));
  assert.ok(ACTIVE_LOCAL_SCRIPTS.indexOf(pullLabSource)<ACTIVE_LOCAL_SCRIPTS.indexOf(defensiveSource));
