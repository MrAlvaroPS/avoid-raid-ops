...
test('active compatibility asset manifest makes the current production stack explicit',()=>{
  ...
  assert.equal(fallbackBridge?.src,'/mechanics-defensives-fallback-bridge-v4.js?v=4.0.0-migration5-owner1');
  assert.equal(fallbackBridge?.owner,'split-source-owners');
  assert.equal(fallbackBridge?.authority,'migration-bridge');
  assert.equal(fallbackBridge?.role,'post-owner-retirement-hold-noop');
  assert.equal(fallbackBridge?.retirement,'retire-after-post-owner-green-validation');
...
test('runtime domains have one primary owner while the monolithic WCL runtime is compatibility-only',()=>{
  ...
  assert.ok(ACTIVE_LOCAL_SCRIPTS.indexOf(fallbackBridge)<ACTIVE_LOCAL_SCRIPTS.indexOf(defensiveSource));
