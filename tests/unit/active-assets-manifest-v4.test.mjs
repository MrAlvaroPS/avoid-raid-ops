...
test('active compatibility asset manifest makes the current production stack explicit',()=>{
  ...
  assert.equal(ACTIVE_LOCAL_SCRIPTS.length,13);
  ...
  assert.equal(ACTIVE_LOCAL_SCRIPTS.some(asset=>asset.id==='mechanics-defensives-fallback-bridge'),false,'retired split bridge must not remain active');
  const pullLabShadow=ACTIVE_LOCAL_SCRIPTS.find(asset=>asset.id==='pull-lab-source-shadow');
  assert.equal(pullLabShadow?.src,'/pull-lab-runtime.js?v=4.0.0-migration7-shadow1');
  assert.equal(pullLabShadow?.owner,'pull-lab-source');
  assert.equal(pullLabShadow?.sourceOwner,'apps/web/src/features/pull-lab/runtime.js');
  assert.equal(pullLabShadow?.authority,'source-shadow');
  assert.equal(pullLabShadow?.role,'pull-lab-parity-shadow-non-authoritative');
  assert.equal(pullLabShadow?.retirement,'promote-after-browser-parity');
  const defensiveSource=ACTIVE_LOCAL_SCRIPTS.find(asset=>asset.id==='defensive-audit-source-runtime');
  assert.equal(defensiveSource?.src,'/defensive-audit-runtime.js?v=4.0.0-migration5-owner1');
  assert.equal(defensiveSource?.sourceOwner,'apps/web/src/features/defensive-audit/runtime.js');
  assert.equal(defensiveSource?.authority,'source-owner');
  assert.equal(defensiveSource?.role,'single-source-defensive-audit-presentation');
  assert.equal(defensiveSource?.retirement,'keep-stable-source-owned-transport');
  const mechanicsSource=ACTIVE_LOCAL_SCRIPTS.find(asset=>asset.id==='mechanics-source-runtime');
  assert.equal(mechanicsSource?.src,'/mechanics-runtime.js?v=4.0.0-migration4-owner1');
...
test('runtime domains have one primary owner while the monolithic WCL runtime is compatibility-only',()=>{
  ...
  assert.ok(ACTIVE_LOCAL_SCRIPTS.indexOf(legacy)<ACTIVE_LOCAL_SCRIPTS.indexOf(pullLabShadow));
  assert.ok(ACTIVE_LOCAL_SCRIPTS.indexOf(pullLabShadow)<ACTIVE_LOCAL_SCRIPTS.indexOf(defensiveSource));
