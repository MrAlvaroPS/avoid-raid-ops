...
test('active compatibility asset manifest makes the current production stack explicit',()=>{
  ...
  assert.equal(fallbackBridge?.src,'/mechanics-defensives-fallback-bridge-v4.js?v=4.0.0-migration5-shadow1');
  assert.equal(fallbackBridge?.owner,'split-source-owners');
  assert.equal(fallbackBridge?.authority,'migration-bridge');
  assert.equal(fallbackBridge?.role,'defensive-source-parity-shadow-trigger');
  const defensiveSource=ACTIVE_LOCAL_SCRIPTS.find(asset=>asset.id==='defensive-audit-source-runtime');
  assert.equal(defensiveSource?.src,'/defensive-audit-runtime.js?v=4.0.0-migration5-shadow1');
  assert.equal(defensiveSource?.sourceOwner,'apps/web/src/features/defensive-audit/runtime.js');
  assert.equal(defensiveSource?.authority,'migration-source-shadow');
  assert.equal(defensiveSource?.role,'defensive-audit-source-parity-shadow');
