...
const baselineEvidencePrefix=(args,signalId,evidenceFingerprint)=>`matched-null-baselines/${corpusId(args)}/${Number(signalId)}/${String(evidenceFingerprint)}/evidence/`;
const empiricalEvidenceFingerprint=episode=>String(episode?.empiricalBuildFingerprint||episode?.matchedNullEvidenceFingerprint||episode?.buildFingerprint||'');
...
async function context(input){
  ...
  const evidenceFingerprint=empiricalEvidenceFingerprint(episode);if(!evidenceFingerprint)throw new Error('Mechanic episode has no empirical evidence fingerprint for matched-null compatibility');
  const [evidenceRecords,wideProfiles,deepProfiles,previousControls]=await Promise.all([persistedAt(`semantic-probes/${corpusId(args)}/evidence/`),persistedAt(`profiles/${corpusId(args)}/`),persistedAt(`deep/${corpusId(args)}/`),persistedAt(baselineEvidencePrefix(args,signalId,evidenceFingerprint))]);
  ...
  plan.empiricalEvidenceFingerprint=evidenceFingerprint;
  plan.interpretationBuildFingerprint=episode.buildFingerprint;
  return{args,signalId,episode,evidenceRecords,plan,evidenceFingerprint};
}
async function previewFor(ctx,input){const cacheRecords=await loadMatchedNullCacheV1({plan:ctx.plan});const preview=buildMatchedNullBaselinePreviewV1({plan:ctx.plan,cacheRecords,maxWclCalls:bounded(input.maxWclCalls,20,1,60),maxContinuationRounds:bounded(input.maxContinuationRounds,1,0,3),eventLimit:bounded(input.eventLimit,1000,100,5000),reservePct:input.minimumRateLimitReservePct,reservePoints:input.minimumRateLimitReservePoints});return{...preview,empiricalEvidenceFingerprint:ctx.evidenceFingerprint,interpretationBuildFingerprint:ctx.episode.buildFingerprint};}
...
export default defineHandler(async event=>{
  ...
  try{
    ...
    if(action==='evaluate'){const cacheRecords=await loadMatchedNullCacheV1({plan:ctx.plan}),evaluation=evaluateMatchedNullBaselineV1({episode:ctx.episode,controlRecords:cacheRecords,config:ctx.plan.config});return json({ok:true,apiVersion:API_VERSION,networkExecuted:false,wclCallsExecuted:0,empiricalEvidenceFingerprint:ctx.evidenceFingerprint,interpretationBuildFingerprint:ctx.episode.buildFingerprint,evaluation});}
