...
async function scope(input){
  const raw=await loadAnyEncounterModel(input);if(!raw)throw new Error('No persisted canonical boss model is available for matched null baseline');
  ...
  if(!args.encounterId||!args.difficulty||!args.partition)throw new Error('Resolved encounter/difficulty/partition is required');return args;
}
...
async function context(input){
  ...
  const [evidenceRecords,wideProfiles,deepProfiles,previousControls]=await Promise.all([
    persistedAt(`semantic-probes/${corpusId(args)}/evidence/`),persistedAt(`profiles/${corpusId(args)}/`),persistedAt(`deep/${corpusId(args)}/`),persistedAt(baselineEvidencePrefix(args,signalId,episodeBuildFingerprint)),
  ]);
  const plan=buildMatchedNullBaselinePlanV1({
    episode,evidenceRecords,profiles:[...wideProfiles,...deepProfiles],rejectedControls:previousControls,
    config:{controlRadiusMs:input.controlRadiusMs,episodeGuardMs:input.episodeGuardMs,candidateOffsetMagnitudesMs:input.candidateOffsetMagnitudesMs,maxControls:input.maxControls,maxControlsPerSource:input.maxControlsPerSource,maxNormalizedFightDistance:input.maxNormalizedFightDistance,minimumMatchedControls:input.minimumMatchedControls,minimumMatchedSources:input.minimumMatchedSources},
  });
  return{args,signalId,episode,evidenceRecords,plan};
}

async function previewFor(ctx,input){
  const cacheRecords=await loadMatchedNullCacheV1({plan:ctx.plan});
  return buildMatchedNullBaselinePreviewV1({plan:ctx.plan,cacheRecords,maxWclCalls:bounded(input.maxWclCalls,20,1,60),maxContinuationRounds:bounded(input.maxContinuationRounds,1,0,3),eventLimit:bounded(input.eventLimit,1000,100,5000),reservePct:input.minimumRateLimitReservePct,reservePoints:input.minimumRateLimitReservePoints});
}
...
export default defineHandler(async event=>{
  ...
  try{
    ...
    const body=await request.json().catch(()=>({})),action=String(body.action||'preview'),input={...body,encounterId:Number(body.encounterId||0),difficulty:Number(body.difficulty||5),partition:Number(body.partition||0),signalId:Number(body.signalId||0)};
    ...
    const preview=await previewFor(ctx,input),supplied=String(body.previewFingerprint||body.fingerprint||'');if(!supplied||supplied!==preview.fingerprint)return json({ok:false,error:'Preview fingerprint is missing or stale; regenerate preview before execution',wclCallsExecuted:0,currentPreview:preview},409);
