...
async function persistedAt(prefix){
  const keys=await corpusList(prefix),rows=[];
...
async function context(input){
  ...
  const [evidenceRecords,wideProfiles,deepProfiles]=await Promise.all([
    persistedAt(`semantic-probes/${corpusId(args)}/evidence/`),
    persistedAt(`profiles/${corpusId(args)}/`),
    persistedAt(`deep/${corpusId(args)}/`),
  ]);
  const plan=buildMatchedNullBaselinePlanV1({
    episode,evidenceRecords,profiles:[...wideProfiles,...deepProfiles],
    config:{
...
async function previewFor(ctx,input){
  ...
  return buildMatchedNullBaselinePreviewV1({
    plan:ctx.plan,cacheRecords,
    maxWclCalls:bounded(input.maxWclCalls,20,1,60),
    maxContinuationRounds:bounded(input.maxContinuationRounds,1,0,3),
    eventLimit:bounded(input.eventLimit,1000,100,5000),
    reservePct:input.minimumRateLimitReservePct,
    reservePoints:input.minimumRateLimitReservePoints,
  });
}
...
export default defineHandler(async event=>{
  ...
  try{
    ...
    const result=await executeMatchedNullBaselineV1({
      plan:ctx.plan,episode:ctx.episode,previewFingerprint:supplied,confirmExecution:true,
      maxWclCalls:bounded(input.maxWclCalls,20,1,60),maxContinuationRounds:bounded(input.maxContinuationRounds,1,0,3),eventLimit:bounded(input.eventLimit,1000,100,5000),reservePct:input.minimumRateLimitReservePct,reservePoints:input.minimumRateLimitReservePoints,
    });
