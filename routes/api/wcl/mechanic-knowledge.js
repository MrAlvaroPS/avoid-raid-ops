...
const API_VERSION='iris-mechanic-knowledge-api-v2';
...
export default defineHandler(async event=>{
  ...
  try{
    ...
    const input={encounterId:Number(url.searchParams.get('encounter')||url.searchParams.get('encounterId')||0),difficulty:Number(url.searchParams.get('difficulty')||5),partition:Number(url.searchParams.get('partition')||0)};
    ...
    if(!result)return json({ok:false,apiVersion:API_VERSION,error:'No official or empirical boss knowledge is persisted for this encounter yet',networkExecuted:false,wclCallsExecuted:0,providerNetworkCallsExecuted:0},404);
