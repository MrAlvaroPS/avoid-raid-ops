...
export default async function handler(req){
  try{
    if(req.method==='GET'){
      const url=new URL(req.url),names=parseNames(url.searchParams.get('difficulties'));
      const preview=await previewRaidCorpusBootstrapV1({difficultyNames:names.length?names:undefined});
      return json(200,{ok:true,action:'preview',networkExecuted:false,wclCallsExecuted:0,wclCombatEventCallsExecuted:0,preview});
    }
    ...
    if(action==='preview'){
      const preview=await previewRaidCorpusBootstrapV1({difficultyNames:difficultyNames.length?difficultyNames:undefined});
      return json(200,{ok:true,action,networkExecuted:false,wclCallsExecuted:0,wclCombatEventCallsExecuted:0,preview});
    }
    if(action==='start'){
