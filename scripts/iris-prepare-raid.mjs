...
async function runPass(passNumber=1){
  ...
  for(const row of rows){
    ...
    if(!state.operational){
      if(status?.status==='paused'){console.log(`PAUSED · ${name} · ${status.phase||'unknown'} · manual inspection required; continuing other automatically safe scopes.`);final.push(compact(row,state));continue;}
      if(!status){status=await startCorpus({...state.scope,corpusProfile:'operational',targetPulls:100,deepTargetPulls:20,maxCandidateReports:500,maxRankingPages:4,maxSourcePages:2});progressed=true;console.log(`START · ${name} · ${availability.difficulty?.name} · ${status.status}/${status.phase}`);}
      while(totalSteps<maxTotal&&bossSteps<maxPerBoss&&status&&(status.status==='running'||status.status==='rate-limited')){
        ...
        status=await stepCorpus({...state.scope,corpusProfile:'operational',targetPulls:100,deepTargetPulls:20,maxCandidateReports:500,maxRankingPages:4,maxSourcePages:2});bossSteps++;totalSteps++;progressed=true;
    ...
    if(rateLimited||totalSteps>=maxTotal){if(rateLimited)console.log('CHECKPOINT · WCL reserve reached; persisted state is safe.');else console.log('CHECKPOINT · pass step budget reached; persisted state is safe.');break;}
  }
