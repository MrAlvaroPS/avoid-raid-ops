...
const fetched=await fetchLootItemV1(itemId),item=fetched.item,players=members.map(row=>({name:row.name,server:row.server?.slug||row.server?.name,region:row.server?.region?.compactName||row.server?.region?.slug||'eu',className:row.className,spec:row.spec,role:row.role||'UNKNOWN',itemLevel:row.itemLevel,character:row.character||{}})),rows=players.map(player=>({player,eligibility:evaluateLootEligibilityV1(item,player)})),eligible=rows.filter(row=>row.eligibility.eligible);
console.log('\nLOOT ROSTER SMOKE');
console.log(JSON.stringify({item:{id:item.id,name:item.name,itemClass:item.itemClass?.name,itemSubclass:item.itemSubclass?.name,inventoryType:item.inventoryType?.type},rosterTotal:players.length,eligible:eligible.length,ineligible:rows.length-eligible.length,networkForRoster:false,simRequested:simulateCount},null,2));
