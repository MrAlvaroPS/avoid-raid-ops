...
console.log('\nLOOT ROSTER SMOKE');
console.log(JSON.stringify({item:{id:item.id,name:item.name,itemClass:item.itemClass?.name,itemSubclass:item.itemSubclass?.name,inventoryType:item.inventoryType?.type},itemProvider:{provider:fetched.provider,source:fetched.source||'blizzard-live',stale:Boolean(fetched.stale),usage:fetched.usage||null},rosterTotal:players.length,eligible:eligible.length,ineligible:ineligible.length,ineligibleReasons:reasonCounts,networkForRoster:false,simRequested:simulateCount},null,2));
