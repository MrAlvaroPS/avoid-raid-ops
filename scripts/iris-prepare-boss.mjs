...
console.log('\nIRIS PREPARE BOSS · operational readiness v1');
...
if(!execute){
  const operational=await loadOperationalEncounterModelV2(scope).catch(()=>null);
  console.log(JSON.stringify({mode:'preview',networkExecuted:false,currentCorpus:status?{status:status.status,phase:status.phase,pulls:status.pullCount,deepPulls:status.deepPullCount,sources:status.sourceStats?.total}:null,liveReadiness:operational?'READY':'NOT READY',operationalReference:operational?.operationalReference||null,nextCommand:`npm run prepare:boss -- --encounter ${boss.wclEncounterId} --difficulty ${availability.difficulty?.name||difficulty} --execute`},null,2));
  process.exit(0);
}
