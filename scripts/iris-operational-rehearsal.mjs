...
console.log(JSON.stringify({mode:'preview',status:preview.status,scope:preview.scope,dataReady:preview.dataReady,mechanicCoverageReady:preview.mechanicCoverageReady,liveReady:preview.liveReady,sampling:preview.sampling,selectedReports:preview.selectedReports,networkExecuted:preview.networkExecuted,networkUpperBound:preview.networkUpperBound,persistedCurrent:Boolean(preview.stored),stored:persisted?{version:persisted.version,status:persisted.status,rehearsalFingerprint:persisted.rehearsalFingerprint||null,coverage:persisted.coverage,checks:persisted.checks,runs:persisted.runs||[]}:null},null,2));
if(!execute){console.log('\nOK: rehearsal diagnostic completed at zero network. Add --execute only when you intentionally want a fresh deterministic rehearsal.');process.exit(0);}
