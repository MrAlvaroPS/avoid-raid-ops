...
### Home guild exclusion

AvoiD is the home guild. The current default guild id is `788166`, configurable through `AVOID_HOME_GUILD_ID`.

A report whose WCL guild id equals the home guild id:

- MUST NOT enter global boss train;
- MUST NOT enter global boss holdout;
- MUST NOT be persisted as a global Wide/Deep profile by new ingestion;
- MUST be purged from legacy global profile caches during canonical 0-WCL rebuild;
...
Only the configured home guild is allowed in this scope.
...
External guild players MUST NOT enter this ledger.
...
- home guild absent from selected global evidence;
...
- any selected home-guild contamination forces maturity to zero.
...
2. reject home-guild profiles;
3. reject wrong-scope profiles;
4. reject reports without trustworthy source identity;
5. source-round-robin the Wide sample;
6. balance the sample using actual pull outcome deficits;
7. restrict Deep selection to reports present in the canonical Wide sample;
8. balance Deep by the same source/outcome policy;
9. rebuild train/holdout aggregate from the selected profiles;
10. compile the model;
11. persist a sampling manifest for audit;
12. purge legacy home-guild profiles from the global-boss profile namespace.

No WCL query is required for those steps.

Cached reports that are not selected are intentionally retained so later policy changes can rebalance the corpus without paying WCL again.
...
3. Home guild report => zero contribution to boss train and holdout.
4. Missing source identity => zero contribution to canonical boss train and holdout.
5. Same source cannot be represented as multiple fake independent sources.
6. Train/holdout never split reports from the same source.
7. Source round-robin keeps selected report counts per active source within one round of each other until sources exhaust.
8. Outcome statistics are pull-level, not report-label-level.
9. Mixed kill/progression reports contribute to every actual fight stratum they contain.
10. Persisted global profiles contain no `friendlyPlayers` list.
11. External guild/player evidence cannot be written through the home-raid scope guard.
12. Recompile/rebalance uses persisted evidence and makes zero WCL requests.
13. Unverified corpus evidence still cannot raise player Reliability directly.
14. Global boss knowledge never stores or derives a Reliability score for an external player.
...
PUBLIC WARCRAFT LOGS
        |
        | many independent guilds/uploaders
        v
