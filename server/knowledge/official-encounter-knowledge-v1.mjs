...
import { persistOfficialEncounterGraphV1 } from './official-encounter-store-v1.mjs';
...
export function buildOfficialEncounterKnowledgePreviewV1(input={}){
  ...
  return {
    ...
    fingerprint,
    request,
    networkUpperBound:{oauthCalls:1,blizzardGameDataCalls:request.journalEncounterId?1:2,wclCalls:0,thirdPartyCalls:0},
    safety:{
      readOnlyProvider:true,
      persistentDerivedRevision:true,
...
function normalizeModes(journal,locale){return (journal?.modes||[]).map(row=>({type:compact(row?.type),name:localizedName(row?.name,locale)})).filter(row=>row.type||row.name);}
function normalizeCreatures(journal,locale){return (journal?.creatures||[]).map(row=>({id:finiteId(row?.id),name:localizedName(row?.name,locale),displayId:finiteId(row?.creature_display?.id)})).filter(row=>row.id||row.name);}
...
export function compileOfficialEncounterGraphV1({journal,locale='en_US',sourceEndpoint=null,namespace=null,wclEncounterId=null}={}){
  ...
  const nodes=[{id:encounterNodeId,type:'encounter',journalEncounterId,wclEncounterId:finiteId(wclEncounterId),name:encounterName,category:compact(journal?.category?.type),instanceId:finiteId(journal?.instance?.id),instanceName:localizedName(journal?.instance?.name,locale)}];
  const edges=[],spellNodes=new Map(),spellIndex=new Map(),sections=[];

  function walk(sectionList,parentSectionId=null,path=[],depth=0){
    for(const section of Array.isArray(sectionList)?sectionList:[]){
      const sectionId=finiteId(section?.id);if(!sectionId)continue;
      const title=localizedName(section?.title,locale),bodyText=localizedName(section?.body_text,locale),role=structuralRole(section,depth),nodeId=`blizzard:journal-section:${sectionId}`,spellId=finiteId(section?.spell?.id),spellName=localizedName(section?.spell?.name,locale),currentPath=[...path,{sectionId,title,role}];
      const sectionNode={id:nodeId,type:'journal-section',sectionId,title,bodyText,depth,structuralRole:role,creatureDisplayId:finiteId(section?.creature_display?.id),spellId,spellName};
      nodes.push(sectionNode);sections.push({...sectionNode,path:currentPath.map(item=>item.title).filter(Boolean)});
      ...
      if(spellId){
        ...
        if(!spellNodes.has(spellId)){const spellNode={id:spellNodeId,type:'spell',abilityId:spellId,name:spellName||null};spellNodes.set(spellId,spellNode);nodes.push(spellNode);}else if(!spellNodes.get(spellId).name&&spellName){spellNodes.get(spellId).name=spellName;}
        ...
        const current=spellIndex.get(spellId)||{abilityId:spellId,name:spellName||null,officialEncounterAssociation:true,memberships:[]};if(!current.name&&spellName)current.name=spellName;current.memberships.push(membership);spellIndex.set(spellId,current);
      }
  ...
  return {
    version:OFFICIAL_ENCOUNTER_KNOWLEDGE_VERSION,schema:OFFICIAL_ENCOUNTER_GRAPH_SCHEMA,fingerprint,
    ...
    encounter:{journalEncounterId,wclEncounterId:finiteId(wclEncounterId),name:encounterName,instanceId:finiteId(journal?.instance?.id),instanceName:localizedName(journal?.instance?.name,locale),category:compact(journal?.category?.type),modes:normalizeModes(journal,locale),creatures:normalizeCreatures(journal,locale)},
    ...
    sections,abilities,
    evidenceContract:{combatTruth:'Warcraft Logs observed combat events remain canonical empirical truth.',officialEncounterSemantics:'Blizzard Encounter Journal is authoritative for the published encounter hierarchy, names, descriptions and spell membership it exposes for this build.',hierarchySemantics:'Journal parent/child placement is official structural metadata; it does not by itself prove event-to-event causality or timing in a pull.',observedOccurrence:false,causalCombatEvidence:false,canonicalDeepContribution:{reports:0,pulls:0},directScoreDelta:0,automaticPromotion:false},
  };
}

export function officialEncounterMembershipForAbilityV1(graph,abilityId){const id=finiteId(abilityId);if(!id)return null;return graph?.abilities?.find(row=>Number(row.abilityId)===id)||null;}

export async function resolveOfficialEncounterKnowledgeV1(input={},options={}){
  const request=normalizeOfficialEncounterKnowledgeRequest(input),preview=buildOfficialEncounterKnowledgePreviewV1(request);
  ...
  if(!journalEncounterId){
    search=await searchBlizzardJournalEncounterV1(request.encounterName,{fetcher:options.fetcher||fetch,accessToken:token.accessToken,region:request.region,locale:request.locale});dataCalls++;
    if(!search.match){const names=search.candidates.slice(0,5).map(row=>`${row.id}:${row.name}`).join(', ');throw new Error(`Blizzard Journal encounter search did not resolve an unambiguous match${names?`; candidates: ${names}`:''}`);}
    journalEncounterId=search.match.id;href=search.match.href;
  }
