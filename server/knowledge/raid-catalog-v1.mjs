...
function normalizeZone(zone={}){
  ...
  return{zoneId:finite(zone.id),name:zone.name||null,frozen:Boolean(zone.frozen),expansion:{id:finite(zone?.expansion?.id),name:zone?.expansion?.name||null},difficulties,partitions,defaultPartition,encounters,raidDifficultySignature,raidLike:Boolean(raidDifficultySignature&&encounters.length>0)};
}

function currentRaid(candidates=[]){const live=candidates.filter(row=>row.raidLike&&!row.frozen),pool=live.length?live:candidates.filter(row=>row.raidLike);return pool.slice().sort((a,b)=>Number(b.expansion?.id||0)-Number(a.expansion?.id||0)||Number(b.zoneId||0)-Number(a.zoneId||0))[0]||null;}
...
export function compileRaidCatalogV1(zones=[]){
  ...
  return{...payload,fingerprint:digest(payload),currentRaid:selected,selection:{policy:'latest-non-frozen-raid-difficulty-signature-by-expansion-and-zone-id',usesCombatLogs:false,usesRankingOutcome:false,hardcodedZoneId:false,reason:selected?(selected.frozen?'No non-frozen raid zone was available; newest raid-like zone selected as fallback.':'Newest non-frozen zone with Normal/Heroic + Mythic raid difficulty signature in the newest expansion.'):'No raid-like WCL zone was available.'},evidenceContract:{metadataOnly:true,wclCombatEventCalls:0,reportRequired:false,automaticPromotion:false}};
}

export async function resolveRaidCatalogV1({graphql=wclGraphql}={}){const data=await graphql(WCL_RAID_CATALOG_QUERY,{});const catalog=compileRaidCatalogV1(data?.worldData?.zones||[]);return{...catalog,usage:{wclMetadataCalls:1,wclCombatEventCalls:0,blizzardCalls:0}};}
