...
function membershipApplicability(membership,map,difficultyId,metadataAvailable){
  if(!metadataAvailable)return{status:'difficulty-applicability-unresolved',applies:true,explicitDifficultyIds:[]};
...
export function compileOfficialEncounterDifficultyViewV1({officialGraph,difficulty,journalDifficultySnapshot=null}={}){
  ...
  const difficultyName=String(difficulty?.name||`Difficulty ${difficultyId}`),metadataAvailable=Boolean(journalDifficultySnapshot);
  ...
  const encounterExplicit=metadataAvailable&&encounterSet.size>0,encounterApplies=!encounterExplicit||encounterSet.has(difficultyId),abilities=[];
  if(encounterApplies){
    for(const ability of officialGraph.abilities||[]){
      const memberships=(ability.memberships||[]).map(membership=>({...membership,difficultyApplicability:membershipApplicability(membership,map,difficultyId,metadataAvailable)})).filter(row=>row.difficultyApplicability.applies);
  ...
  const payload={
    ...
    applicability:{encounterStatus:!metadataAvailable?'difficulty-applicability-unresolved':encounterExplicit?(encounterApplies?'explicitly-applicable':'explicitly-not-applicable'):'no-explicit-encounter-restriction',encounterExplicitDifficultyIds:[...encounterSet].sort((a,b)=>a-b),sectionDifficultyMetadataAvailable:metadataAvailable},
    abilities,sections,
    ...
    evidenceContract:{difficultyScoped:true,difficultyApplicabilityMayBeUnresolved:!metadataAvailable,crossDifficultyComparisonForbidden:true,crossDifficultyEmpiricalReuse:false,observedOccurrence:false,causalCombatEvidence:false,automaticPromotion:false},
  };
