...
export const MATCHED_NULL_BASELINE_DEFAULTS=Object.freeze({
  ...
  episodeGuardMs:2500,
  candidateOffsetMagnitudesMs:[12000,18000,24000,30000],
  maxControls:10,
...
function targetTimestamps(evidenceRecords=[],signalId){
  ...
  for(const row of evidenceRecords||[]){
    ...
    for(const events of Object.values(row.streams||{}))for(const event of events||[]){
      ...
      if(fight!=null){add(row.reportCode,fight,event?.timestamp);continue;}
...
function candidateOffsets(magnitudes,anchorIndex){
  const signFirst=anchorIndex%2===0?1:-1,out=[];
  for(const magnitude of magnitudes){const value=Math.abs(Number(magnitude));out.push(signFirst*value,-signFirst*value);}
  return out;
}
...
function viableControl({anchor,anchorIndex,fight,knownTargets,selected,config,episodeRadius}){
  ...
  for(const offset of candidateOffsets(config.candidateOffsetMagnitudesMs,anchorIndex)){
    const center=Number(anchor.anchorTimestamp)+offset,windowStart=center-config.controlRadiusMs,windowEnd=center+config.controlRadiusMs;
    ...
    return{
      source:anchor.source,reportCode:anchor.reportCode,fightID:anchor.fightID,referenceTimestamp:center,windowStart,windowEnd,windowMs:config.controlRadiusMs,anchorTimestamp:anchor.anchorTimestamp,
...
function configFrom(input={}){
  ...
  return{
    ...MATCHED_NULL_BASELINE_DEFAULTS,...input,
    ...
    candidateOffsetMagnitudesMs:magnitudes.length?magnitudes:[...MATCHED_NULL_BASELINE_DEFAULTS.candidateOffsetMagnitudesMs],
...
export function buildMatchedNullBaselinePlanV1({episode,evidenceRecords=[],profiles=[],config:configInput={}}={}){
  ...
  return{
    version:MATCHED_NULL_BASELINE_V1_VERSION,policyVersion:MATCHED_NULL_BASELINE_POLICY_V1_VERSION,planFingerprint:digest(planPayload),
    episodeId:episode.episodeId,episodeBuildFingerprint:episode.buildFingerprint,scope:episode.scope,signalId,config,episodeRadiusMs:episodeRadius,
    anchorsAvailable:anchors.length,plannedControls:controls.length,plannedSources:plannedSources.size,sufficientByPlan:controls.length>=config.minimumMatchedControls&&plannedSources.size>=config.minimumMatchedSources,
    deficits,controls,
    evidenceContract:{sameFight:true,sameOutcome:true,phaseMatchedWhenAvailable:true,localFlankControlsUsed:false,episodeOverlapForbidden:true,exactFightIDsOnly:true,targetSignalContaminationMustBeRejected:true,canonicalDeepContribution:{reports:0,pulls:0},directScoreDelta:0,automaticPromotion:false},
  };
}
...
export function buildMatchedNullBaselinePreviewV1({plan,cacheRecords=[],maxWclCalls=20,maxContinuationRounds=1,eventLimit=1000,reservePct=.18,reservePoints=600}={}){
  ...
  const complete=new Set((cacheRecords||[]).filter(row=>row?.pagination?.complete===true).map(row=>String(row.controlId))),partial=new Set((cacheRecords||[]).filter(row=>row&&row?.pagination?.complete!==true).map(row=>String(row.controlId))),pending=plan.controls.filter(row=>!complete.has(String(row.controlId)));
  ...
  return{
    version:MATCHED_NULL_BASELINE_PREVIEW_V1_VERSION,fingerprint:digest({previewVersion:MATCHED_NULL_BASELINE_PREVIEW_V1_VERSION,planFingerprint:plan.planFingerprint,config}),dryRun:true,executesWcl:false,wclCallsExecuted:0,
    ...
    evidenceContract:plan.evidenceContract,controls:plan.controls,config,
  };
}
...
export function evaluateMatchedNullBaselineV1({episode,controlRecords=[],config:configInput={}}={}){
  ...
  return{
    version:MATCHED_NULL_BASELINE_V1_VERSION,policyVersion:MATCHED_NULL_BASELINE_POLICY_V1_VERSION,episodeId:episode.episodeId,episodeBuildFingerprint:episode.buildFingerprint,
    baselineSufficient,completeControls:complete.length,invalidControls:allComplete.length-complete.length,matchedSources:sources.size,minimumMatchedControls:config.minimumMatchedControls,minimumMatchedSources:config.minimumMatchedSources,
    patternAssessments:rows,summary:{supported:rows.filter(row=>row.status==='matched-specificity-supported').length,noise:rows.filter(row=>row.status==='matched-background-noise').length,partial:rows.filter(row=>row.status==='matched-specificity-partial').length,insufficient:rows.filter(row=>row.status==='matched-baseline-insufficient').length},
    ...
    evidenceContract:{sameFightControls:true,localFlankControlsUsed:false,targetSignalContaminationRejected:true,sourceIndependenceNotYetClaimed:true,statisticalStabilityNotYetClaimed:true,holdoutNotYetClaimed:true,canonicalDeepContribution:{reports:0,pulls:0},directScoreDelta:0},
  };
}
