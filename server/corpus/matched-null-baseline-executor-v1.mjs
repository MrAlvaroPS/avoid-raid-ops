...
function compactStreams(streams={}){return Object.fromEntries(Object.entries(streams).map(([stream,events])=>[stream,(events||[]).map(event=>compactEvent(event,stream))]));}
...
function evidenceRecord(plan,control,bundle){
  ...
  return{version:MATCHED_NULL_BASELINE_STORAGE_V1_VERSION,kind:'matched-null-control',signalId:Number(plan.signalId),episodeId:plan.episodeId,episodeBuildFingerprint:plan.episodeBuildFingerprint,controlId:String(control.controlId),source:String(control.source),reportCode:String(control.reportCode),fightID:Number(control.fightID),referenceTimestamp:Number(control.referenceTimestamp),windowMs:Number(control.windowMs),windowStart:Number(control.windowStart),windowEnd:Number(control.windowEnd),anchorTimestamp:Number(control.anchorTimestamp),match:control.match,
    streams:compact,pagination:bundle?.pagination||null,rateLimit:bundle?.rateLimit||null,validNull:!contaminated,invalidReason:contaminated?'target-signal-observed-inside-control':null,
  ...
  if(pending.length){
    try{
      ...
      for(const control of pending){
        const key=matchedNullControlEvidenceKey(plan,control),partial=partialById.get(String(control.controlId)),resumeBundle=partial?{streams:partial.streams,pagination:partial.pagination,rateLimit:partial.rateLimit}:null;
        const bundle=await fetchSemanticEventBundle({code:control.reportCode,fightIDs:[control.fightID],abilityID:null,windowStart:control.windowStart,windowEnd:control.windowEnd,limit:preview.config.eventLimit,maxContinuationRounds:preview.config.maxContinuationRounds,runQuery,resumeBundle,
        ...
        const finalRecord=evidenceRecord(plan,control,bundle);await storageSet(key,finalRecord);if(bundle?.pagination?.complete===true){completeById.set(String(control.controlId),finalRecord);partialById.delete(String(control.controlId));}else partialById.set(String(control.controlId),finalRecord);
      }
    }catch(error){if(error instanceof MatchedNullBudgetStop){status='budget-capped';stopReason=error.reason;}else throw error;}
  }

  const cache=await loadMatchedNullCacheV1({plan,storageGet}),evaluation=evaluateMatchedNullBaselineV1({episode,controlRecords:cache,config:plan.config}),completeControls=cache.filter(row=>row?.pagination?.complete===true).length,validCompleteControls=cache.filter(row=>row?.pagination?.complete===true&&row?.validNull!==false).length,invalidControls=cache.filter(row=>row?.pagination?.complete===true&&row?.validNull===false).length,partialControls=cache.filter(row=>row?.pagination?.complete!==true).length;
