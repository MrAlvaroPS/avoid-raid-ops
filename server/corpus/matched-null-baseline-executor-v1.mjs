...
function evidenceFingerprint(plan){return String(plan?.empiricalEvidenceFingerprint||plan?.episodeBuildFingerprint||'');}
function baseKey(plan){return`matched-null-baselines/${corpusId(plan.scope)}/${Number(plan.signalId)}/${evidenceFingerprint(plan)}`;}
...
export async function executeMatchedNullBaselineV1({plan,episode,previewFingerprint,confirmExecution=false,maxWclCalls=20,maxContinuationRounds=1,eventLimit=1000,reservePct=.18,reservePoints=600,fetcher=wclGraphql,storageGet=corpusGet,storageSet=corpusSet}={}){
  ...
  const result={version:MATCHED_NULL_BASELINE_EXECUTOR_V1_VERSION,storageVersion:MATCHED_NULL_BASELINE_STORAGE_V1_VERSION,previewFingerprint:preview.fingerprint,planFingerprint:plan.planFingerprint,episodeId:plan.episodeId,episodeBuildFingerprint:plan.episodeBuildFingerprint,empiricalEvidenceFingerprint:evidenceFingerprint(plan),scope:plan.scope,signalId:Number(plan.signalId),status,stopReason,wclCallsExecuted:calls,rateLimit:rate,lastQuery,
    plannedControls:plan.plannedControls,completeControls,validCompleteControls,invalidControls,partialControls,evaluation,
    evidenceContract:{matchedSameFightControls:true,pairedAnchorComparison:true,anchorContextCoversEpisodeRadius:true,controlCoversEpisodeRadius:true,targetSignalContaminationRejected:true,targetSignalGuardRadiusValidated:true,innerControlEventsOnly:true,localFlankControlsUsed:false,rawActorIdsPersisted:false,rawActorNamesPersisted:false,semanticReinterpretationMayReuseEvidence:true,canonicalDeepContribution:{reports:0,pulls:0},directScoreDelta:0,automaticPromotion:false,sourceIndependenceNotYetClaimed:true,holdoutNotYetClaimed:true}};
