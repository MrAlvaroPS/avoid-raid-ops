...
function configFrom(input={}){
  return{
    maxWclCalls:Math.max(1,Math.min(80,Number(input.maxWclCalls)||UNTOUCHED_HOLDOUT_ACQUISITION_DEFAULTS.maxWclCalls)),
    maxFightIDsPerSource:Math.max(1,Math.min(12,Number(input.maxFightIDsPerSource)||UNTOUCHED_HOLDOUT_ACQUISITION_DEFAULTS.maxFightIDsPerSource)),
    maxPairsPerSource:Math.max(1,Math.min(3,Number(input.maxPairsPerSource)||UNTOUCHED_HOLDOUT_ACQUISITION_DEFAULTS.maxPairsPerSource)),
    maxAnchorContinuationRounds:Math.max(0,Math.min(3,Number(input.maxAnchorContinuationRounds)??UNTOUCHED_HOLDOUT_ACQUISITION_DEFAULTS.maxAnchorContinuationRounds)),
    maxContextContinuationRounds:Math.max(0,Math.min(2,Number(input.maxContextContinuationRounds)??UNTOUCHED_HOLDOUT_ACQUISITION_DEFAULTS.maxContextContinuationRounds)),
    eventLimit:Math.max(100,Math.min(3000,Number(input.eventLimit)||UNTOUCHED_HOLDOUT_ACQUISITION_DEFAULTS.eventLimit)),
...
export async function loadUntouchedHoldoutAcquisitionCacheV1({reservation,scope,storageGet=corpusGet}={}){
  const rows=[];for(const source of reservation?.reservedSources||[]){const value=await storageGet(untouchedHoldoutSourceEvidenceKeyV1(reservation,scope,source.source)).catch(()=>null);if(value)rows.push(value);}return rows;
}
...
export function buildUntouchedHoldoutAcquisitionPreviewV1({reservation,episode,cacheRecords=[],config:configInput={}}={}){
  ...
  const complete=new Set((cacheRecords||[]).filter(sourceComplete).map(row=>sourceKey(row.source))),pending=(reservation?.reservedSources||[]).filter(row=>!complete.has(sourceKey(row.source)));
  const executable=Boolean(ready&&missingSeeds.length===0&&pending.length>0);
  const status=!ready?'reservation-not-ready':missingSeeds.length?'reserved-source-seed-metadata-missing':pending.length===0?'complete-from-cache':'acquisition-ready';
  ...
  return{
    ...
    cache:{completeSources:complete.size,pendingSources:pending.length},
...
function compactFight(row){return{id:Number(row.id),startTime:Number(row.startTime),endTime:Number(row.endTime),kill:Boolean(row.kill),fightPercentage:finite(row.fightPercentage),bossPercentage:finite(row.bossPercentage),phaseTransitions:(row.phaseTransitions||[]).map(item=>({id:finite(item?.id),startTime:finite(item?.startTime)})).filter(item=>item.startTime!=null)};}
...
async function fetchHeader(code,scope,runQuery){const data=await runQuery(CORPUS_REPORT_HEADER_QUERY,{code:String(code),encounter:Number(scope.encounterId),difficulty:Number(scope.difficulty)},{kind:'holdout-report-header',code:String(code)});const report=data?.reportData?.report;if(!report)return{report:null,rateLimit:data?.rateLimitData||null};return{report:{code:String(report.code||code),guild:report.guild||null,owner:report.owner||null,fights:(report.fights||[]).map(compactFight)},rateLimit:data?.rateLimitData||null};}
...
export async function executeUntouchedHoldoutAcquisitionV1({reservation,episode,scope,previewFingerprint,confirmExecution=false,config:configInput={},fetcher=wclGraphql,storageGet=corpusGet,storageSet=corpusSet,clock=()=>Date.now()}={}){
  ...
  if(preview.status==='complete-from-cache')return{version:UNTOUCHED_HOLDOUT_ACQUISITION_V1_VERSION,status:'complete',reusedCompletedEvidence:true,wclCallsExecuted:0,holdoutEvidence:{reservationFingerprint:reservation.fingerprint,collectedAt:Math.max(Number(reservation.reservedAt)||0,clock()),scope,sources:cacheBefore.filter(sourceComplete).map(row=>({source:row.source,patterns:row.patterns}))},sourceRecords:cacheBefore};
  if(!preview.executable)throw new Error(`Holdout combat acquisition is not executable: ${preview.status}`);

  const config=preview.config,complete=new Map(cacheBefore.filter(sourceComplete).map(row=>[sourceKey(row.source),row]));
  ...
  try{
    ...
    for(const reserved of reservation.reservedSources||[]){
      const source=sourceKey(reserved.source);if(complete.has(source))continue;
      ...
      const header=existing?.header||await fetchHeader(code,scope,runQuery);const report=header?.report||header;
      if(!report?.fights?.length){const row={version:UNTOUCHED_HOLDOUT_ACQUISITION_STORAGE_V1_VERSION,status:'inconclusive-no-eligible-fight',source,seedReportCode:code,reservationFingerprint:reservation.fingerprint,collectedAt:clock(),patterns:[]};await storageSet(key,row);continue;}
      ...
      const row={version:UNTOUCHED_HOLDOUT_ACQUISITION_STORAGE_V1_VERSION,status:validPairs>0?'complete':'inconclusive-no-valid-pair',source,seedReportCode:code,reservationFingerprint:reservation.fingerprint,episodeId:episode.episodeId,signalId:Number(episode.anchor.abilityId),collectedAt:clock(),selectedFightIDs,pairs:pairs.map(row=>({controlId:row.controlId,anchorTimestamp:row.anchorTimestamp,controlTimestamp:row.controlTimestamp,fightID:row.fightID,valid:row.valid,invalidReason:row.invalidReason})),patterns,evidenceContract:{sourceWasReserved:true,seedReportWasFrozenBeforeCombatEvidence:true,sourceExpansionUsed:false,pairedSameFightNull:true,rawActorIdsPersisted:false,rawActorNamesPersisted:false,newCandidateDiscovery:false,newSourceDiscovery:false,automaticPromotion:false}};
      await storageSet(key,row);if(sourceComplete(row))complete.set(source,row);
    }
  ...
  const cacheAfter=await loadUntouchedHoldoutAcquisitionCacheV1({reservation,scope,storageGet}),completeRows=cacheAfter.filter(sourceComplete),holdoutEvidence={reservationFingerprint:reservation.fingerprint,collectedAt:Math.max(Number(reservation.reservedAt)||0,...completeRows.map(row=>Number(row.collectedAt)||0),clock()),scope,sources:completeRows.map(row=>({source:row.source,patterns:row.patterns}))};
  if(status==='complete'&&completeRows.length<(reservation.config?.minimumEvaluableSources||3)){status='evidence-incomplete';stopReason='insufficient-evaluable-sources';}
  const result={version:UNTOUCHED_HOLDOUT_ACQUISITION_V1_VERSION,storageVersion:UNTOUCHED_HOLDOUT_ACQUISITION_STORAGE_V1_VERSION,previewFingerprint:preview.fingerprint,reservationFingerprint:reservation.fingerprint,episodeId:episode.episodeId,scope,signalId:Number(episode.anchor.abilityId),status,stopReason,wclCallsExecuted:calls,rateLimit:rate,lastQuery,completeSources:completeRows.length,reservedSources:(reservation.reservedSources||[]).length,holdoutEvidence,evidenceContract:{reservationFrozenBeforeCombat:true,onlyFrozenCandidatesQueried:true,onlyReservedSourcesQueried:true,onlyFrozenSeedReportsQueried:true,sourceExpansionForbidden:true,pairedSameFightNull:true,rawActorIdsPersisted:false,rawActorNamesPersisted:false,canonicalDeepContribution:{reports:0,pulls:0},directScoreDelta:0,automaticPromotion:false}};
