...
export const OPERATIONAL_EXECUTION_CONTRACT_VERSION='operational-execution-wcl-streams-v2';
...
export async function loadOperationalReadinessV1(input={}){
  ...
  const stored=await corpusGet(readinessKey(scope));if(!stored)return null;
  const contract=String(stored.operationalExecutionContractVersion||'');
  if(contract&&contract!==OPERATIONAL_EXECUTION_CONTRACT_VERSION)return{...stored,status:'rehearsal-required',mechanicCoverageReady:false,liveReady:false,staleReason:'operational-execution-contract-changed'};
  return stored;
}
...
export async function previewOperationalRehearsalV1(input={}){
  ...
  const legacyRequest={version:OPERATIONAL_READINESS_VERSION,scope:{encounterId:Number(scope.encounterId),difficulty:Number(scope.difficulty),partition:Number(scope.partition)},operationalReferenceVersion:scope.operational.operationalReference?.version||null,operationalModelFingerprint,samplingPolicyVersion:manifest?.policyVersion||null,selectedReports};
  const request={...legacyRequest,operationalExecutionContractVersion:OPERATIONAL_EXECUTION_CONTRACT_VERSION};
  const fingerprint=digest(request),legacyFingerprint=digest(legacyRequest),storedFingerprint=String(stored?.rehearsalFingerprint||''),storedContract=String(stored?.operationalExecutionContractVersion||'');
  const legacyLiveReadyCompatible=!storedContract&&stored?.status==='live-ready'&&stored?.liveReady===true&&storedFingerprint===legacyFingerprint;
  const storedCurrent=storedContract===OPERATIONAL_EXECUTION_CONTRACT_VERSION?storedFingerprint===fingerprint:legacyLiveReadyCompatible;
  return{version:OPERATIONAL_READINESS_VERSION,operationalExecutionContractVersion:OPERATIONAL_EXECUTION_CONTRACT_VERSION,status:storedCurrent?(stored?.status||'rehearsal-required'):'rehearsal-required',fingerprint,operationalModelFingerprint,scope:request.scope,dataReady:true,mechanicCoverageReady:storedCurrent&&stored?.mechanicCoverageReady===true,liveReady:storedCurrent&&stored?.liveReady===true,operationalReference:scope.operational.operationalReference,sampling:{wideReports:Number(manifest?.wide?.reports||0),wideSources:Number(manifest?.wide?.sources||0),deepReports:Number(manifest?.deep?.reports||0),deepSources:Number(manifest?.deep?.sources||0)},selectedReports,packDiagnostics:packDiagnostics(scope.operational?.pack,aggregate),stored:storedCurrent?stored:null,storedPrevious:storedCurrent?null:(stored||null),legacyLiveReadyCompatible,networkExecuted:false,networkUpperBound:{operationalExecutionRuns:selectedReports.length},evidenceContract:{deterministicCanonicalReports:true,reportSelectionUsesPerformance:false,externalReportsNeverEnterHomeExecution:true,rehearsalDoesNotTrain:true,rehearsalDoesNotPromote:true,sameDifficultyOnly:true,staleCoverageReviewCannotBlockChangedModel:true,executionContractVersionedReadiness:true,legacyCoverageReviewInvalidatedOnStreamV2:true,legacyLiveReadyCompatible:true,packDiagnosticsFromPersistedAggregate:true,wcl429NeverBecomesCoverageEvidence:true}};
}
