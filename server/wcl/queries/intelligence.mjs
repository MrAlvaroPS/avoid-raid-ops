...
query AvoidEncounterIntelligence(
 ...
 mechanicDamage:events(dataType:DamageTaken,fightIDs:$all,hostilityType:Friendlies,filterExpression:$damageFilter,limit:10000,useAbilityIDs:true,useActorIDs:true){data nextPageTimestamp}
 mechanicCasts:events(dataType:Casts,fightIDs:$all,hostilityType:Enemies,filterExpression:$castFilter,limit:10000,useAbilityIDs:true,useActorIDs:true){data nextPageTimestamp}
...
query AvoidMechanicDamagePage($code:String!,$all:[Int],$damageFilter:String!,$start:Float){
 reportData{report(code:$code,allowUnlisted:true){
  mechanicDamage:events(dataType:DamageTaken,fightIDs:$all,hostilityType:Friendlies,filterExpression:$damageFilter,startTime:$start,limit:10000,useAbilityIDs:true,useActorIDs:true){data nextPageTimestamp}
 }}}
`;
...
query AvoidFeatherDebuffPage($code:String!,$all:[Int],$featherFilter:String!,$start:Float){
 reportData{report(code:$code,allowUnlisted:true){
  feather:events(dataType:Debuffs,fightIDs:$all,hostilityType:Friendlies,filterExpression:$featherFilter,startTime:$start,limit:10000,useAbilityIDs:true,useActorIDs:true){data nextPageTimestamp}
 }}}
`;
