...
query AvoidTelemetryMeta($code:String!){reportData{report(code:$code,allowUnlisted:true){code title startTime endTime revision visibility zone{id name} guild{id name} owner{id} masterData{actors{id name type subType server} abilities{gameID name icon type}} fights(killType:Encounters){id encounterID name difficulty kill startTime endTime fightPercentage bossPercentage averageItemLevel inProgress lastPhaseAsAbsoluteIndex phaseTransitions{id startTime} friendlyPlayers friendlySpecs friendlyItemLevels wipeCalledTime}}}}`;
