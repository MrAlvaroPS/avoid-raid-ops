...
export function normalizePlayers({report,fight,bestSummary,bestDamageDone,bestHealing,bestDamageTaken,allCasts,interruptEvents=[],dispelEvents=[],deathAnalysis=null,profiles=new Map(),encounterPulls=0}){
  ...
  return (fight.friendlyPlayers||[]).map((id,idx)=>{
    ...
    const base={actorId:Number(id),name:actor.name||roleData.name||`Actor ${id}`,className:roleData.className||actor.subType||actor.type||null,spec:roleData.spec||fight.friendlySpecs?.[idx]||null,itemLevel:num(fight.friendlyItemLevels?.[idx]),role:roleData.role||null};
    ...
    const reliability={value:null,status:'pending',confidence:'unknown',reason:'Mechanic and defensive rule-pack dimensions are not yet calculated.'};
    return {
