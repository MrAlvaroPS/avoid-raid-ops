...
export const LIVE_RL_DIAGNOSTIC_VERSION='live-rl-diagnostic-v3';
...
function deathStory(pull){
  const rows=(pull?.meaningfulDeathTimeline?.length?pull.meaningfulDeathTimeline:pull?.rawDeathTimeline)||[],first=rows[0]||pull?.firstDeath||null,firstMs=finite(pull?.firstDeathMs)?Number(pull.firstDeathMs):finite(first?.fightRelativeMs)?Number(first.fightRelativeMs):null,duration=finite(pull?.durationMs)?Number(pull.durationMs):null;
...
function band(current,dist){
  if(!finite(current)||!dist)return null;const x=Number(current);
  if(finite(dist.p95)&&x>Number(dist.p95))return'above-p95';if(finite(dist.p90)&&x>Number(dist.p90))return'above-p90';if(finite(dist.p75)&&x>Number(dist.p75))return'above-p75';if(finite(dist.p25)&&x<Number(dist.p25))return'below-p25';return'typical';
}
...
function globalPhrase(row){
  const g=row?.global;if(!g)return null;const label=g.band==='above-p95'?'above GLOBAL P95':g.band==='above-p90'?'above GLOBAL P90':g.band==='above-p75'?'above GLOBAL P75':g.band==='below-p25'?'below GLOBAL P25':'within the GLOBAL middle range';
  const mean=finite(g.mean)?` · GLOBAL mean ${Number(g.mean).toFixed(1)} ${g.unit}`:'';return`${label}${mean} · ${g.reports} canonical Deep reports`;
}
...
export async function getLiveRlDiagnosticV1({reportCode,encounterId,difficulty,telemetry,fightId=null}={}){
  const pull=selectedPull(telemetry,fightId);if(!pull?.fightId)return null;
  const selectedFightId=Number(pull.fightId),[raw,knowledge,globalReference]=await Promise.all([
  ...
  const report=raw?.reportData?.report||{},official=officialAbilityMap(knowledge),benchmarks=benchmarkMap(globalReference),damageTable=report.currentDamageTaken;
