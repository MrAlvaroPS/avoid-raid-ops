...
function annotate(row,official){const matches=official.get(Number(row.abilityId))||[];return{...row,officialMechanics:matches};}
...
export async function getLiveRlDiagnosticV1({reportCode,encounterId,difficulty,telemetry}={}){
  ...
  const casts=groupAbilities(uniqueCasts(paginatorEvents(report.currentEnemyCasts))).slice(0,8).map(row=>annotate(row,official));
  const debuffEvents=paginatorEvents(report.currentDebuffs).filter(e=>['applydebuff','applydebuffstack','refreshdebuff'].includes(eventType(e))),debuffs=groupAbilities(debuffEvents).slice(0,8).map(row=>annotate(row,official));
  ...
  const result={version:LIVE_RL_DIAGNOSTIC_VERSION,generatedAt:Date.now(),scope:{reportCode,encounterId:Number(encounterId),difficulty:Number(difficulty),fightId},pull:{pullNumber:latest.pullNumber,kill:Boolean(latest.kill),fightPercentage:latest.fightPercentage??null,durationMs:latest.durationMs??null,stageCount:latest.stageCount??null},wipeStory,incomingPressure:{totalMappedDamage:totalIncoming,topAbilities:incoming},castPressure:{enemyCasts:casts,totalUniqueCasts:casts.reduce((s,r)=>s+Number(r.count||0),0)},debuffPressure:{topDebuffs:debuffs,totalDebuffEvents:debuffEvents.length},control:{interrupts:interrupts.length,dispels:dispels.length},playerPressure,reviewPriorities:priorities({latest,wipeStory,incoming,casts,debuffs,interrupts:interrupts.length,dispels:dispels.length}),officialKnowledge:{available:Boolean(knowledge?.bossKnowledge?.officialMechanics?.length),mechanics:Number(knowledge?.bossKnowledge?.officialMechanics?.length||0),networkExecuted:false},pagesIncomplete:{enemyCasts:Boolean(report.currentEnemyCasts?.nextPageTimestamp),interrupts:Boolean(report.currentInterrupts?.nextPageTimestamp),dispels:Boolean(report.currentDispels?.nextPageTimestamp),debuffs:Boolean(report.currentDebuffs?.nextPageTimestamp)},evidenceContract:{selectedPullExact:true,sameDifficultyOnly:true,objectiveObservedFactsOnly:true,noMechanicBlameWithoutClassifier:true,noMissedInterruptInference:true,noMissedDispelInference:true,officialMappingIsSemanticContextNotCausality:true}};
  return result;
}
