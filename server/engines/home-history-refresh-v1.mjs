...
export const AVOID_HISTORY_REFRESH_VERSION='avoid-history-refresh-v1.1';
const DAY=86400000;
...
async function listGuildRaidReports({guildId,zoneId,days=180,maxPages=5}){
  ...
  while(hasMore&&page<=Math.max(1,Math.min(20,Number(maxPages)||5))){const data=await wclGraphql(LIST_GUILD_REPORTS_QUERY,{guildId:Number(guildId),start,end,zoneId:Number(zoneId),limit:100,page});const result=data?.reportData?.reports;if(!result)break;total=result.total??total;rows.push(...(result.data||[]));hasMore=Boolean(result.has_more_pages);page++;}
...
export async function refreshPersistedAvoidHistoryV1({guildId=homeGuildId(),days=180,maxPages=5,maxChangedReports=40,concurrency=3}={}){
  ...
  const existingRoster=await loadHomeRosterV1({guildId}).catch(()=>null),rosterBase=existingRoster||{version:HOME_ROSTER_STORE_VERSION,guildId:Number(guildId),guild:{id:Number(guildId),name:null},temporary:true,source:'home-history-bootstrap',fetchedAt:null,members:[]},raidRoster=mergeHistoryRosterV1(rosterBase,allReports,{syncedAt});await persistHomeRosterV1(raidRoster,{guildId});
  const rosterSummary={source:'wcl-home-history-participant',activeRaiders:Number(raidRoster?.raidRoster?.activeMembers||0),totalRaidPulls:Number(raidRoster?.raidRoster?.totalRaidPulls||0),networkCallsAdded:0};
  const index={version:AVOID_HISTORY_STORE_VERSION,refreshVersion:AVOID_HISTORY_REFRESH_VERSION,status,guildId:Number(guildId),zone:{id:zoneId,name:catalog.currentRaid?.name||null},catalogFingerprint:catalog.fingerprint||null,syncedAt,highWaterMark,reports:summaries,roster:rosterSummary,refresh:{explicit:true,listedReports:listed.reports.length,changedReports:changed.length,updatedReports:updated.length,remainingChangedReports:remaining,errors:errors.slice(0,10),window:listed.window,pagination:listed.pagination,networkCalls:listed.networkCalls+selected.length}};
