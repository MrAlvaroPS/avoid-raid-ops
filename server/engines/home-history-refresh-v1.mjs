...
async function listGuildRaidReports({guildId,zoneId,days=180,maxPages=5}){
  const end=Date.now()+DAY,start=end-Math.max(7,Math.min(730,Number(days)||180))*DAY,rows=[],specs=new Map();let page=1,hasMore=true,total=null;
  while(hasMore&&page<=Math.max(1,Math.min(20,Number(maxPages)||5))){const data=await wclGraphql(LIST_GUILD_REPORTS_QUERY,{guildId:Number(guildId),start,end,zoneId:Number(zoneId),limit:100,page});const result=data?.reportData?.reports;if(!result)break;total=result.total??total;rows.push(...(result.data||[]));for(const spec of specDirectoryFromGameData(data?.gameData))specs.set(Number(spec.id),spec);hasMore=Boolean(result.has_more_pages);page++;}
  const byCode=new Map();for(const row of rows)if(row?.code&&Number(row?.zone?.id)===Number(zoneId))byCode.set(String(row.code),row);
  return{reports:[...byCode.values()].sort((a,b)=>Number(a.startTime)-Number(b.startTime)),specDirectory:[...specs.values()].sort((a,b)=>Number(a.id)-Number(b.id)),window:{start,end,days},pagination:{pagesScanned:page-1,total:total??byCode.size,truncated:hasMore},networkCalls:page-1};
}
