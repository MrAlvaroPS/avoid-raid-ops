...
function attendanceDirectory(reports){
  ...
  for(const report of reports||[]){
    for(const actor of report?.masterData?.actors||[]){
      ...
      if(!directory.has(key))directory.set(key,{key,name:actor.name||null,className:actor.subType||actor.type||null,server:actor.server||null});
    }
  }
  return directory;
}

function asFightReports(reports,field){return (reports||[]).map(r=>({...r,fights:r?.[field]||[]})).filter(r=>r.fights.length);}
function stripRosterKeys(pull){if(!pull||typeof pull!=='object')return pull;const {rosterKeys,...rest}=pull;return rest;}
...
export async function getGuildHistory({reportCode,guildId,encounterId,daysBefore=35,daysAfter=7}){
  ...
  const current=currentData?.reportData?.report;if(!current)return null;
  const selected=selectEncounter(current.fights,encounterId);const anchor=selected[0];
  if(!anchor)return{generatedAt:Date.now(),engineVersion:'3.8.5',guildId,zone:current.zone,encounter:null,nights:[],recentNights:[],progressionPulls:[],progressModel:null,playerAttendance:{status:'pending',scope:'raid-zone-history-window',players:[]},currentNight:null,previousNight:null,delta:null,pagination:{total:0,hasMore:false,candidatesScanned:0}};

  const DAY=86400000,start=Number(current.startTime)-daysBefore*DAY,end=Number(current.endTime)+daysAfter*DAY;
  ...
  // Progress keeps the analytical encounter population. Attendance uses every
  // closed boss pull: an early reset proves attendance but must not affect Progress.
  ...
  const playerAttendance={
    ...
    scope:'raid-zone-history-window',zoneId:Number(current.zone.id),historyWindowDays:daysBefore,windowStart:start,windowEnd:end,
  ...
  const rawProgressionPulls=clustered.flatMap(n=>(n.progressionPulls||[]).map(p=>({...p,sessionId:n.sessionId,sessionIndex:n.sessionIndex,sessionStartTime:n.startTime,sessionTitle:n.title}))).sort((a,b)=>Number(a.absoluteStartTime)-Number(b.absoluteStartTime));
  ...
  const nights=progressModel.nights||[],recent=nights.slice(-5);
  ...
  return{
    generatedAt:Date.now(),engineVersion:'3.8.5',guildId,zone:current.zone,
    encounter:{id:anchor.encounterID,name:anchor.name,difficulty:anchor.difficulty},historyWindow:{daysBefore,daysAfter,start,end},
    progressionPulls,progressModel,playerAttendance,nights,recentNights:recent,currentNight,previousNight,delta,
    reportDiagnostics:reports.map(r=>({reportCode:r.code,title:r.title,startTime:r.startTime,endTime:r.endTime,encounterPulls:(r.encounterFights||[]).filter(f=>!f.inProgress).length,raidBossPulls:(r.raidFights||[]).filter(f=>!f.inProgress).length})),
    ...
    evidence:{nightProgress:'confirmed',progressionPullSeries:'canonical-deduped-from-history-reports',progressMetrics:'server-derived-single-source-v2-data-integrity',progressMetricEligibility:'versioned-and-auditable',playerAttendance:'canonical-deduped-all-closed-raid-boss-pulls-since-first-indexed-appearance',playerAttendanceScope:'raid-zone-history-window',playerAttendanceJoinDate:'not-claimed-by-wcl',queryStrategy:'two-stage-paginated-dual-population',sessionClustering:'time-window',pullDeduplication:'timestamp+duration+progress',historyWindowDays:daysBefore,resetBoundary:'not-calculated',rosterCausality:'not-calculated'}
  };
}