...
/**
 * Build a compact, deduplicated raid-attendance model from canonical sessions.
 * `rosterKeys` must already have been unioned across duplicate logger views.
 *
 * WCL cannot prove a guild join date, so the denominator begins at the first
 * indexed appearance of the character inside the bounded raid-history window.
 */
export function buildPlayerAttendance(sessions=[],directory=new Map()){
  ...
  for(const row of byPlayer.values()){
    ...
    players.push({
      ...
      source:'canonical-deduplicated-raid-history'
    });
  }
