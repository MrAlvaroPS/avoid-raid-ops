...
export function normalizeGuildRosterMemberV1(row={},classMap=new Map(),{fetchedAt=Date.now()}={}){
  ...
  return{
    ...
    spec:null,specId:null,role:null,itemLevel:null,actorId:null,character:null,reliability:null,
...
function observedPayload(row={}){
  return {
    actorId:finite(row?.actorId),name:String(row?.name||'').trim(),className:String(row?.className||row?.class||'').trim()||null,spec:String(row?.spec||'').trim()||null,specId:finite(row?.specId),role:String(row?.role||'').trim().toUpperCase()||null,
...
function blankHistoryMember(activity={}){
  return{id:null,canonicalId:null,name:activity.name,classId:null,className:activity.className||null,classSlug:null,level:null,guildRank:null,hidden:false,server:{id:null,name:null,slug:null,normalizedName:null,region:{id:null,name:null,slug:null,compactName:null}},spec:null,specId:null,role:null,itemLevel:null,actorId:null,character:null,reliability:null,directory:null,raidActivity:null,observed:null};
}

function normalizeSpecDirectory(specDirectory=[]){
  const byId=new Map();
  for(const row of specDirectory||[]){const id=finite(row?.id);if(id)byId.set(id,{id,name:String(row?.name||'').trim()||null,slug:String(row?.slug||'').trim()||null,classId:finite(row?.classId),className:String(row?.className||'').trim()||null});}
  return byId;
}

export function mergeHistoryRosterV1(roster={},reports=[],{syncedAt=Date.now(),specDirectory=[]}={}){
  const activities=new Map(),specs=normalizeSpecDirectory(specDirectory);let totalRaidPulls=0;
  for(const report of reports||[]){
    ...
    for(const fight of report?.fights||[]){
      ...
      const at=Number(report?.startTime||0)+Number(fight?.startTime||0),pullKey=`${report.reportCode}:${Number(fight.id)}`,friendly=(fight?.friendlyPlayers||[]).map(Number),friendlySpecs=Array.isArray(fight?.friendlySpecs)?fight.friendlySpecs:[],friendlyIlvls=Array.isArray(fight?.friendlyItemLevels)?fight.friendlyItemLevels:[];
      friendly.forEach((actorId,index)=>{
  ...
  for(const [key,activity] of activities){let idx=byName.get(key);if(idx==null){idx=members.length;byName.set(key,idx);members.push(blankHistoryMember(activity));}
    const base=members[idx],pulls=activity.pullKeys.size,reportsCount=activity.reportCodes.size,preferObserved=Boolean(base?.observed);
    members[idx]={...base,name:activity.name||base.name,className:base.className||activity.className||null,spec:preferObserved?(base.spec||activity.spec):(activity.spec||base.spec),specId:preferObserved?(base.specId??activity.specId):(activity.specId??base.specId),role:preferObserved?(base.role||activity.role):(activity.role||base.role),itemLevel:preferObserved?(base.itemLevel??activity.itemLevel):(activity.itemLevel??base.itemLevel),raidActivity:{source:HOME_ROSTER_SOURCE_HISTORY,temporary:true,confirmedFromHomeLogs:true,pulls,reports:reportsCount,reportCodes:[...activity.reportCodes].sort(),firstSeenAt:activity.firstSeenAt,lastSeenAt:activity.lastSeenAt,totalRaidPulls,attendancePct:totalRaidPulls>0?100*pulls/totalRaidPulls:null,syncedAt:Number(syncedAt)||Date.now(),latestProfileAt:activity.profileSeenAt,specId:activity.specId,spec:activity.spec,role:activity.role,itemLevel:activity.itemLevel}};
  }
  const next={...roster,version:HOME_ROSTER_STORE_VERSION,specDirectory:[...specs.values()].sort((a,b)=>Number(a.id)-Number(b.id)),members:members.sort((a,b)=>String(a.name).localeCompare(String(b.name))),raidRoster:{source:HOME_ROSTER_SOURCE_HISTORY,temporary:true,totalRaidPulls,activeMembers:activities.size,syncedAt:Number(syncedAt)||Date.now()},updatedAt:Date.now()};
  next.fingerprint=fingerprint({guild:next.guild,raidRoster:next.raidRoster,members:next.members.map(row=>({key:identityKey(row),name:row.name,className:row.className,spec:row.spec,specId:row.specId,role:row.role,itemLevel:row.itemLevel,directory:row.directory,raidActivity:row.raidActivity,observed:row.observed}))});return next;
}
...
export function mergeObservedRosterV1(roster={},players=[],{observedAt=Date.now(),reportCode=null,fightId=null}={}){
  ...
  for(const raw of players||[]){
    ...
    members[idx]={...base,name:obs.name||base.name,className:obs.className||base.className,spec:obs.spec||base.spec,specId:obs.specId??base.specId,role:obs.role||base.role,itemLevel:obs.itemLevel??base.itemLevel,actorId:obs.actorId??base.actorId,character:obs.character||base.character,reliability:obs.reliability||base.reliability,server:{...(base.server||{}),name:serverName,slug:base?.server?.slug||serverName,region:{...(base?.server?.region||{}),name:regionName,compactName:base?.server?.region?.compactName||regionName}},observed:{source:HOME_ROSTER_SOURCE_OBSERVED,temporary:false,observedAt:Number(observedAt)||Date.now(),reportCode:reportCode||null,fightId:finite(fightId)}};
  }
...
export function mergeDirectoryRosterV1(existing={},incoming={}){
  const oldByName=new Map((existing?.members||[]).map(row=>[norm(row.name),row])),members=(incoming?.members||[]).map(row=>{
    ...
    return {...row,spec:old.spec||row.spec,specId:old.specId??row.specId,role:old.role||row.role,itemLevel:old.itemLevel??row.itemLevel,actorId:old.actorId??row.actorId,character:old.character||row.character,reliability:old.reliability||row.reliability,raidActivity:old.raidActivity||row.raidActivity,observed:old.observed||row.observed};
  });
  ...
  const next={...incoming,version:HOME_ROSTER_STORE_VERSION,specDirectory:existing?.specDirectory||incoming?.specDirectory||[],raidRoster:existing?.raidRoster||incoming?.raidRoster||null,members:members.sort((a,b)=>String(a.name).localeCompare(String(b.name))),updatedAt:Date.now()};
  next.fingerprint=fingerprint({guild:next.guild,raidRoster:next.raidRoster,members:next.members.map(row=>({key:identityKey(row),name:row.name,className:row.className,spec:row.spec,specId:row.specId,role:row.role,itemLevel:row.itemLevel,directory:row.directory,raidActivity:row.raidActivity,observed:row.observed}))});
  return next;
}
