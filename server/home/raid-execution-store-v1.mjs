...
function pullFact(row={}){return{fightId:Number(row.fightId),pullNumber:finite(row.pullNumber),kill:Boolean(row.kill),fightPercentage:finite(row.fightPercentage),durationMs:finite(row.durationMs)};}
...
export function buildHomeExecutionSnapshotV1(data,{reportCode,guildId=homeGuildId()}={}){
  ...
  const mechanics=(data?.mechanics?.mechanics||[]).map(mechanicRow).filter(row=>row.key),failures=(data?.mechanics?.failures||[]).map(failureRow).filter(row=>row.mechanicKey&&Number.isFinite(row.fightId)),eligibleFightIds=(data?.analysisPopulation?.eligibleFightIds||[]).map(Number).filter(Number.isFinite),eligible=new Set(eligibleFightIds),pulls=(data?.telemetry?.pullIntelligence?.pulls||[]).map(pullFact).filter(row=>eligible.has(Number(row.fightId)));
  const evidence={reportCode:code,encounterId,difficulty,eligibleFightIds,mechanics,failures,pulls,blocker:data?.blocker||null,nextPullCalls:data?.nextPullCalls||[],rulePack:data?.rulePack||null,operationalReference:data?.operationalReference||null};
  const fingerprint=digest(evidence);
  return{version:HOME_RAID_EXECUTION_VERSION,fingerprint,generatedAt:Number(data.generatedAt)||Date.now(),guildId:Number(guildId),reportCode:code,report:data?.report||{code},encounter:{id:encounterId,name:data?.encounter?.name||null,difficulty,difficultyName:data?.encounter?.difficultyName||null,scopeKey:`${encounterId}:d${difficulty}`},analysisPopulation:data?.analysisPopulation||null,pulls,mechanics,failures,blocker:data?.blocker||null,nextPullCalls:data?.nextPullCalls||[],rulePack:data?.rulePack||null,operationalReference:data?.operationalReference||null,evidenceContract:{homeOnly:true,derivedFromObservedWcl:true,scopeIdentity:'encounter+difficulty',crossDifficultyAggregationForbidden:true,latestPullDoesNotReplaceHistory:true,clearStateSeparateFromMechanicalMaturity:true,automaticPromotion:false}};
}
...
function pullOrder(snapshots=[]){
  const rows=[];
  for(const snapshot of snapshots){const ids=(snapshot?.analysisPopulation?.eligibleFightIds||[]).map(Number).filter(Number.isFinite),facts=new Map((snapshot?.pulls||[]).map(row=>[Number(row.fightId),row]));ids.forEach((fightId,index)=>{const fact=facts.get(Number(fightId))||{};rows.push({key:`${snapshot.reportCode}:${fightId}`,reportCode:snapshot.reportCode,fightId,index,at:Number(snapshot?.report?.startTime||snapshot.generatedAt||0)+index,snapshotAt:Number(snapshot.generatedAt||0),kill:Boolean(fact.kill),fightPercentage:finite(fact.fightPercentage),durationMs:finite(fact.durationMs)});});}
...
export function aggregateHomeRaidExecutionV1(snapshots=[],{guildId=homeGuildId(),encounterId,difficulty,recentWindow=8}={}){
  ...
  const latest=valid.slice().sort((a,b)=>Number(a.generatedAt)-Number(b.generatedAt)).at(-1)||null,nextPullCalls=cleared?[]:mechanics.filter(m=>(m.recentFailurePullRate??0)>0).slice(0,3).map(m=>({kind:'mechanic',confidence:m.linkedDeaths>0?'high':'medium',title:m.name,detail:m.expectedAction||`${m.recentFailedPulls}/${recent.length||0} recent pulls contained this classified failure.`}));
  return{version:HOME_RAID_EXECUTION_VERSION,status:pulls.length?'ready':'empty',generatedAt:Date.now(),guildId:Number(guildId),encounter:{id:Number(encounterId),name:latest?.encounter?.name||null,difficulty:Number(difficulty),difficultyName:latest?.encounter?.difficultyName||null,scopeKey:`${Number(encounterId)}:d${Number(difficulty)}`},progression:{status:cleared?'CLEARED':pulls.length?'PROGRESSION':'NO AVOID DATA',cleared,kills,bestRemainingPct},state:{key:state.toLowerCase().replace(/\s+/g,'-'),label:state,tone,mechanicsGate,mechanicalAccuracyPct,scoreSemantics:'aggregate clean mechanic occurrences / observed mechanic opportunities; never a single-pull score'},population:{reports:valid.length,pulls:pulls.length,recentPulls:recent.length,previousPulls:previous.length,normalizedMechanics:normalized.length,totalMechanics:mechanics.length},blocker:blocker?{key:blocker.key,name:blocker.name,recentFailurePullRate:blocker.recentFailurePullRate,previousFailurePullRate:blocker.previousFailurePullRate,trend:blocker.trend,linkedDeaths:blocker.linkedDeaths,expectedAction:blocker.expectedAction}:null,mechanics,nextPullCalls:nextPullCalls.length?nextPullCalls:(cleared?[]:(latest?.nextPullCalls||[]).filter(call=>['mechanic','player-focus','secondary-mechanic'].includes(String(call?.kind||''))).slice(0,3)),latestReportCode:latest?.reportCode||null,evidenceContract:{homeOnly:true,longitudinalAcrossAllPersistedPulls:true,recentWindowForCurrentState:window,singlePullCannotReplaceAggregate:true,clearStateSeparateFromMechanicalMaturity:true,mechanicallyReadyIsNotOverallKillability:true,automaticPromotion:false},networkExecuted:false,wclCallsExecuted:0};
}
