...
import { simcWorkerStatusV1 } from '../loot/simc-runner-v1.mjs';
import { simulateLootRaidMatrixV1 } from '../loot/simc-matrix-v1.mjs';
...
export default async req=>{
  try{
    ...
    if(req.method==='GET'){
      if(url.searchParams.get('state')==='1'){const ledger=await loadLootLedgerV1(),worker=await simcWorkerStatusV1();return jsonResponse(200,{ok:true,version:'loot-api-v1.4',worker,ledger,lootCounts:lootCountsV1(ledger),networkExecuted:false},'private, no-store');}
      if(url.searchParams.get('raidCatalog')==='1'){const result=await ensureRaidLootCatalogV1({refresh:url.searchParams.get('refresh')==='1'});return jsonResponse(200,{ok:true,version:'loot-api-v1.4',...result},'private, no-store');}
      const itemId=Number(url.searchParams.get('item'));if(Number.isInteger(itemId)&&itemId>0){const result=await fetchLootItemV1(itemId);return jsonResponse(200,{ok:true,version:'loot-api-v1.4',...result},'private, no-store');}
      const q=String(url.searchParams.get('q')||'').trim();if(q){const result=url.searchParams.get('global')==='1'?await searchLootItemsV1(q,{limit:Number(url.searchParams.get('limit'))||12}):await searchRaidLootCatalogV1(q,{limit:Number(url.searchParams.get('limit'))||30,refresh:url.searchParams.get('refresh')==='1'});return jsonResponse(200,{ok:true,version:'loot-api-v1.4',...result},'private, no-store');}
    ...
    if(req.method==='POST'){
      ...
      if(action==='simulate'){
        ...
        if(!eligible.length)return jsonResponse(200,{ok:true,version:'loot-api-v1.4',item,eligibility,simulation:{results:[],status:'no-spec-compatible-raiders'},evidenceContract:{simGainOnly:true,automaticAward:false,raidOnly:true,physicalEligibilitySeparateFromSpecFit:true}},'private, no-store');
        const simulation=await simulateLootRaidMatrixV1({players:eligible,item,itemLevel:body?.itemLevel,iterations:body?.iterations||1000,concurrency:body?.concurrency||1});
        return jsonResponse(200,{ok:true,version:'loot-api-v1.4',item,eligibility,simulation,evidenceContract:{simGainOnly:true,automaticAward:false,raidOnly:true,physicalEligibilitySeparateFromSpecFit:true,simRequiresNoKnownSpecContradiction:true,healerAndTankRaidValueNotFabricated:true,observedCombatantInfoPreferred:true,armoryFallbackAllowed:true,unsupportedSimcSpecsRemainEligible:true,unitargetAndFiveTargetSeparated:true,mixIsNeutralEqualWeightNotBossTruth:true}},'private, no-store');
      }
      if(action==='eligibility'){
        const item=await resolveItem(body),players=safePlayers(body?.players);return jsonResponse(200,{ok:true,version:'loot-api-v1.4',item,rows:players.map(player=>({player,eligibility:evaluateLootEligibilityV1(item,player)}))},'private, no-store');
      }
      if(action==='award'){const result=await awardLootV1(body);return jsonResponse(200,{ok:true,version:'loot-api-v1.4',...result},'private, no-store');}
      if(action==='remove-award'){const result=await removeLootAwardV1(body?.id);return jsonResponse(200,{ok:true,version:'loot-api-v1.4',...result},'private, no-store');}
  ...
  }catch(error){return jsonResponse(500,{ok:false,version:'loot-api-v1.4',error:error instanceof Error?error.message:String(error)},'no-store');}
};
