...
export default async req=>{
  ...
  try{
    ...
    if(readiness?.liveReady!==true){
      ...
      // telemetry remains safe and useful in Live/Pull Lab, so return it without invoking the
      // operational mechanic engine or persisting HOME mechanical conclusions.
      const telemetry=await getTelemetry({reportCode,...scope}).catch(()=>null);
      return jsonResponse(200,{ok:true,version:'operational-execution-gate-v3',generatedAt:Date.now(),status:'boss-reference-not-ready',reason:'operational-rehearsal-required',telemetry,mechanics:null,blocker:null,nextPullCalls:[],homeExecution:null,readiness:readiness?{status:readiness.status,dataReady:readiness.dataReady===true,mechanicCoverageReady:readiness.mechanicCoverageReady===true,liveReady:false,coverage:readiness.storedPrevious?.coverage||readiness.stored?.coverage||null,operationalExecutionContractVersion:readiness.operationalExecutionContractVersion||null}:null,evidenceContract:{dataReadyDoesNotImplyLiveReady:true,noUnrehearsedMechanicClassification:true,currentRehearsalFingerprintRequired:true,sameDifficultyOnly:true,safeTelemetryAllowedWhileMechanicsGated:true,gatedTelemetryDoesNotPersistHomeMechanics:true}},'private, no-store');
    }
    const result=await getOperationalExecutionV1({reportCode,...scope});if(!result)return jsonResponse(404,{ok:false,error:'Report not found',reportCode},'no-store');return jsonResponse(200,{ok:true,...result,readiness:{status:readiness.status,liveReady:true,coverage:readiness.stored?.coverage||readiness.storedPrevious?.coverage||null,operationalExecutionContractVersion:readiness.operationalExecutionContractVersion||null}},'private, no-store');
  }
