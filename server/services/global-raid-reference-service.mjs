...
export async function getGlobalRaidReferenceV1({encounterId,difficulty,partition=0}={}){
  ...
  const corpus=await getCorpusStatus({encounterId:encounter,difficulty:diff,partition:part}).catch(()=>null);
  if(!corpus)return{version:GLOBAL_RAID_REFERENCE_VERSION,scope:{encounterId:encounter,difficulty:diff,partition:part||null},status:'not-started',maturity:'none',reference:null,networkExecuted:false,evidenceContract:{sameDifficultyOnly:true,foundationIsAcceptedKnowledge:false,automaticPromotion:false}};
  const ready=['ready','completed'].includes(String(corpus.status)),building=['running','rate-limited','paused'].includes(String(corpus.status));
  const maturity=ready?'foundation-ready':building?'foundation-building':'foundation-incomplete';
