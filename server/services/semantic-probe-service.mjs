...
import { corpusGet, corpusList, corpusStorageErrorInfo } from '../corpus/storage.mjs';
import { applyBossSamplingPolicyV380 } from '../corpus/model-policy-v380.mjs';
import { buildSemanticSurgicalProbePlanV2 } from '../corpus/semantic-surgical-probe-planner-v2.mjs';
import {
...
export default async function semanticProbeService(request){
  ...
  try{
    ...
    const result=await executeSemanticProbePlanV1({
      plan:row.plan,
      previewFingerprint:supplied,
      confirmExecution:true,
      config:row.config,
      deepProfiles:row.deepProfiles,
    });
