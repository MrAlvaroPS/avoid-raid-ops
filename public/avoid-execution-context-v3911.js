...
function pullOptions(){const pulls=state.history?.pulls||[{key:'all',mode:'all',label:'All pulls'}];return pulls.map(row=>`<option value="${esc(row.key)}">${esc(row.label||row.key)}</option>`).join('');}
...
function maintainShell(){render();setRaidBreadcrumb();}
function mount(){reveal();maintainShell();publish();void loadHistory();setInterval(maintainShell,750);}
window.__AVOID_EXECUTION__=Object.freeze({release:RELEASE,getState:currentContext,history:Object.freeze({reload:loadHistory,refresh:refreshHistory}),activeReport:Object.freeze({load:startOrLoad,stop:stopPolling,poll:pollActive,hydrate:hydrateActiveExecution}),selectPull,remount:maintainShell});
