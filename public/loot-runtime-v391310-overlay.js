...
function ensureStyle(){
  ...
  .avoid-loot-root .loot-wowhead-fallback{display:flex;align-items:center;justify-content:center;width:30px;height:30px;font:800 11px/1 system-ui;color:#7f9aa6;background:linear-gradient(135deg,#101b20,#071014)}
...
function iconAnchor({itemId,itemLevel=null,label='World of Warcraft item'}={}){
  ...
  const a=document.createElement('a');a.className='loot-wowhead-icon-link';a.href=wowheadHref(itemId,itemLevel);a.target='_blank';a.rel='noreferrer';a.title=label;a.setAttribute('aria-label',label);a.setAttribute('data-wowhead',`item=${Number(itemId)}${finite(itemLevel)?`&ilvl=${Number(itemLevel)}`:''}`);a.setAttribute('data-wh-iconize-link','true');a.setAttribute('data-wh-icon-size','small');a.setAttribute('data-wh-rename-link','false');
...
function decorateSelected(root){
  ...
  const existing=q('a[href*="wowhead.com/item"]',selected),parsed=parseItem(existing?.href||''),itemId=parsed.itemId;if(!itemId)return;
  const heading=q('h3',selected);if(!heading||heading.dataset.lootIconized==='1')return;
  const label=heading.textContent?.trim()||`Item ${itemId}`,ilvl=manualIlvl(root),anchor=iconAnchor({itemId,itemLevel:ilvl,label});if(!anchor)return;
  heading.textContent='';heading.classList.add('loot-selected-icon-row');heading.dataset.lootIconized='1';heading.append(anchor);
  if(ilvl){const badge=document.createElement('span');badge.className='loot-selected-sim-ilvl';badge.textContent=`SIM ILVL ${ilvl}`;badge.title='Exact item level sent to SimulationCraft for both ST and MT5';heading.append(badge);}
}
