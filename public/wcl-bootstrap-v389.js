...
const DIFFICULTY_SCOPED_PATHS=new Set(['/api/wcl/report','/api/wcl/status','/api/wcl/telemetry','/api/wcl/history','/api/wcl/intelligence','/api/wcl/corpus']);
...
function locationDifficulty(){const value=Number(new URLSearchParams(location.search).get('difficulty')||0);return Number.isInteger(value)&&value>0?value:null;}
function scopedGetInput(input,url,method){
  if(!url||url.origin!==location.origin||method!=='GET'||!DIFFICULTY_SCOPED_PATHS.has(url.pathname)||url.searchParams.has('difficulty'))return input;
  const difficulty=locationDifficulty();if(!difficulty)return input;
  url.searchParams.set('difficulty',String(difficulty));
  if(input instanceof Request)return new Request(url.toString(),{method:'GET',headers:input.headers,credentials:input.credentials,cache:input.cache,redirect:input.redirect,referrer:input.referrer,referrerPolicy:input.referrerPolicy,integrity:input.integrity,keepalive:input.keepalive,mode:input.mode});
  return url.toString();
}
...
window.fetch=function avoidWclFetch(input,init={}){
  const originalUrl=requestUrl(input),method=requestMethod(input,init),scopedInput=scopedGetInput(input,originalUrl,method),url=requestUrl(scopedInput),timeout=url&&url.origin===location.origin&&method==='GET'?GET_TIMEOUTS[url.pathname]:null;
  if(!timeout||init?.signal)return nativeFetch(scopedInput,init);
  const controller=new AbortController(),startedAt=Date.now(),timer=setTimeout(()=>controller.abort(new DOMException(`AvoiD WCL request timeout after ${timeout}ms`,'TimeoutError')),timeout);
  record(url.pathname,{state:'pending',startedAt,timeoutMs:timeout,difficulty:url.searchParams.get('difficulty')||null});
  return nativeFetch(scopedInput,{...init,signal:controller.signal}).then(response=>{record(url.pathname,{state:'complete',status:response.status,durationMs:Date.now()-startedAt,difficulty:url.searchParams.get('difficulty')||null});return response;}).catch(error=>{const timedOut=controller.signal.aborted;record(url.pathname,{state:timedOut?'timeout':'error',durationMs:Date.now()-startedAt,error:error instanceof Error?error.message:String(error),difficulty:url.searchParams.get('difficulty')||null});throw error;}).finally(()=>clearTimeout(timer));
};
