(() => {
  ...
  function restoreCorpusActions(){
    for(const button of actionButtons()){
      ...
      button.disabled=button.dataset.irisWasDisabled==='1';button.style.pointerEvents='';button.style.opacity='';
      delete button.dataset.irisStorageDisabled;delete button.dataset.irisWasDisabled;
    }
  }

  function renderStorageIssue(){
    const panel=corpusPanel();if(!panel)return;
    let alert=panel.querySelector('.iris-storage-alert');
    if(!storageIssue){alert?.remove();corpusRoot()?.classList.remove('iris-storage-blocked');restoreCorpusActions();return;}
    if(!alert){
      alert=document.createElement('div');alert.className='iris-storage-alert';
      const host=corpusRoot()||panel;
      if(host===panel){const title=panel.querySelector('.panel-title');title?.insertAdjacentElement('afterend',alert);}else host.prepend(alert);
    }
    ...
    corpusRoot()?.classList.add('iris-storage-blocked');disableCorpusActions();
  }
  ...
  function renderActionIssue(){
    ...
    if(!alert){alert=document.createElement('div');alert.className='iris-action-alert';const title=panel.querySelector('.panel-title');title?.insertAdjacentElement('afterend',alert);}
  ...
  function rememberStorageFailure(payload,status){
    ...
    if(payload?.code==='CORPUS_BLOB_READ_BLOCKED'||status===403||/Vercel Blob: Failed to fetch blob: 403 Forbidden/i.test(message)||/Blob content access is blocked \(403 Forbidden\)/i.test(message)){
      storageIssue={at:Date.now(),message,storage:payload?.storage||null};actionIssue=null;renderStorageIssue();renderActionIssue();return true;
    }
    return false;
  }

  function requestInfo(args){
    const first=args[0],init=args[1]||{},raw=typeof first==='string'?first:first?.url||'';
    const url=new URL(String(raw||location.href),location.origin),method=String(init.method||first?.method||'GET').toUpperCase();
    let action=url.searchParams.get('action')||'';
    if(method!=='GET'&&init.body){try{action=JSON.parse(String(init.body))?.action||action;}catch{}}
    return{url,method,action:String(action||'corpus').toUpperCase(),isCorpus:url.pathname==='/api/wcl/corpus'};
  }

  function blockedResponse(){
    return new Response(JSON.stringify({ok:false,code:'CORPUS_BLOB_READ_BLOCKED',error:'Iris corpus storage is blocked by the current Vercel Blob usage limit. Reload the page after storage access is restored to retry.',storage:storageIssue?.storage||{kind:'vercel-blob-private',corpusReset:false}}),{status:503,headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store'}});
  }

  const nativeFetch=window.fetch.bind(window);
  window.fetch=async (...args)=>{
    let info;try{info=requestInfo(args);}catch{info={isCorpus:false,method:'GET',action:'CORPUS'};}
    if(info.isCorpus&&storageIssue)return blockedResponse();
    let response;
    try{response=await nativeFetch(...args);}catch(error){
      if(info.isCorpus&&info.method!=='GET'){actionIssue={at:Date.now(),action:info.action,status:0,code:'NETWORK_ERROR',message:String(error?.message||error||'Network request failed')};renderActionIssue();}
      throw error;
    }
    ...
    try{
      ...
      if(!response.ok){
        if(!rememberStorageFailure(payload,response.status)&&info.method!=='GET'){
          actionIssue={at:Date.now(),action:info.action,status:response.status,code:String(payload?.code||'CORPUS_ACTION_FAILED'),message:String(payload?.error||`Corpus action failed with HTTP ${response.status}`)};renderActionIssue();
        }
      }else if(info.method!=='GET'){actionIssue=null;renderActionIssue();}
    }catch{}
    return response;
  };
