...
// Recovery only: do not repaint healthy Safe Telemetry on a timer. If another renderer removes
// the safe shell without a context event, restore it on the next watchdog tick.
setInterval(()=>{
  if(page()!=='live')return;
...
window.__AVOID_LIVE_SAFE_FALLBACK__=Object.freeze({release:RELEASE,render,sameFrameRaceResolution:true,unconditionalRepaint:false});
})();
