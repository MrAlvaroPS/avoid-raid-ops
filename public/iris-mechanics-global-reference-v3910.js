(()=>{
'use strict';
const RELEASE='3.9.10.6',KNOWLEDGE='/api/wcl/mechanic-knowledge';
let token=0,lastKey='';
...
function paint(target,ref,difficultyName){
  if(!target)return;const maturity=String(ref?.maturity||'none'),row=ref?.reference||null;
  ...
  if(maturity==='foundation-building'){cls='partial';badge='BUILDING';value=`${fmt(row?.pulls)} PULLS`;meta=`${fmt(row?.sources)} public sources · ${fmt(row?.deepPulls)} deep · preliminary same-difficulty reference`;}
  else if(maturity==='foundation-ready'){cls='ready';badge='FOUNDATION READY';value=`${fmt(row?.sources)} SOURCES`;meta=`${fmt(row?.pulls)} pulls · ${fmt(row?.deepPulls)} deep · operational comparison, not accepted mechanic knowledge`;}
...
async function refresh(){
  if(!page())return;const target=card(),s=scope();if(!target||!s.journal||!s.difficulty)return;const current=`${s.journal}:d${s.difficulty}`;if(current===lastKey&&target.dataset.irisGlobalReferenceRelease===RELEASE)return;lastKey=current;const mine=++token;
  try{const url=new URL(KNOWLEDGE,location.origin);url.searchParams.set('journal',String(s.journal));url.searchParams.set('difficulty',String(s.difficulty));const res=await fetch(url,{headers:{accept:'application/json'}}),payload=await res.json().catch(()=>null);if(mine!==token||!page())return;if(!res.ok||!payload?.ok)return;const data=payload.result||payload,ref=data?.sources?.globalReference||null;paint(card(),ref,data?.difficulty?.name);}
  catch{}
}
