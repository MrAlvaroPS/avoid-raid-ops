...
function corpusCountdown(resumeAt) {
  const t = Number(resumeAt); if (!Number.isFinite(t)) return null;
  const ms = Math.max(0, t - Date.now()); const total = Math.ceil(ms / 1000);
...
function corpusContext() {
  const id = Number(intelligence?.encounter?.id || payload?.encounter?.id || encounterId);
  if (!Number.isFinite(id) || id <= 0) return null;
  const difficulty = Number(payload?.encounter?.difficulty || payload?.encounter?.difficultyId || intelligence?.encounter?.difficulty || 5) || 5;
  return { encounterId: id, difficulty, partition: 0 };
}

async function corpusRequest(action, extra = {}) {
  const ctx = corpusContext();
  if (!ctx) throw new Error("Encounter unavailable");
  const response = await fetch(corpusEndpoint, {
    method: "POST",
    headers: { "Accept": "application/json", "Content-Type": "application/json" },
    body: JSON.stringify({ action, ...ctx, ...extra })
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok || data?.ok === false) throw new Error(data?.error || `HTTP ${response.status}`);
  corpusState = data.status || null;
  applyCorpusWorkbench();
  return corpusState;
}

async function refreshCorpusStatus(force = false) {
  const ctx = corpusContext();
  if (!ctx || corpusFetching) return;
  if (!force && corpusLoadedEncounter === ctx.encounterId && corpusState !== null) return;
  corpusFetching = true;
  try {
    const url = new URL(corpusEndpoint);
    url.searchParams.set("encounter", String(ctx.encounterId));
    url.searchParams.set("difficulty", String(ctx.difficulty));
    url.searchParams.set("partition", String(ctx.partition));
    if (force) url.searchParams.set("_", String(Date.now()));
    const response = await fetch(url, { headers: { "Accept": "application/json" } });
    const data = await response.json().catch(() => ({}));
    if (!response.ok || data?.ok === false) throw new Error(data?.error || `HTTP ${response.status}`);
    corpusState = data.status || null;
    corpusLoadedEncounter = ctx.encounterId;
    if (corpusState?.targetPulls || corpusState?.targetReports) corpusTargetReports = Number(corpusState.targetPulls || corpusState.targetReports) || corpusTargetReports;
  } catch (error) {
    console.warn("[AvoiD encounter corpus]", error);
  } finally {
    corpusFetching = false;
    applyCorpusWorkbench();
    if (["running", "rate-limited"].includes(corpusState?.status) && !corpusDriving) queueMicrotask(() => pollCorpus());
  }
}

async function pollCorpus() {
  if (corpusDriving) return;
  corpusDriving = true;
  applyCorpusWorkbench();
  try {
    while (corpusDriving) {
      const state = corpusState;
      if (!state || ["ready", "paused"].includes(state.status)) break;
      await sleep(5000);
      if (!corpusDriving) break;
      await refreshCorpusStatus(true);
    }
  } catch (error) {
    console.warn("[AvoiD encounter corpus poller]", error);
  } finally {
    corpusDriving = false;
    applyCorpusWorkbench();
  }
...
function corpusCell(label, value, meta) {
  const div = document.createElement("div");
  const l = document.createElement("label"); l.textContent = label;
  const b = document.createElement("b"); b.textContent = value;
  const s = document.createElement("small"); s.textContent = meta;
  div.append(l, b, s); return div;
}

function corpusButton(label, handler, className = "") {
  const button = document.createElement("button");
  button.type = "button"; button.textContent = label; button.className = className;
  button.addEventListener("click", async event => {
    event.preventDefault(); event.stopPropagation();
    try { button.disabled = true; await handler(); }
    catch (error) { console.warn("[AvoiD encounter corpus action]", error); }
    finally { button.disabled = false; applyCorpusWorkbench(); }
  });
  return button;
}

function applyCorpusWorkbench() {
  const catalogue = panelByTitle("Encounter mechanic catalogue");
  let panel = document.querySelector(".corpus-workbench");
  if (!catalogue) { if (panel) panel.style.display = "none"; return; }
  panel ||= document.createElement("article");
  if (!panel.isConnected) { panel.className = "panel corpus-workbench"; catalogue.insertAdjacentElement("beforebegin", panel); }
  panel.style.display = "";
  panel.replaceChildren();

  const ctx = corpusContext();
  if (ctx && corpusLoadedEncounter !== ctx.encounterId && !corpusFetching) {
    corpusState = null;
    corpusLoadedEncounter = ctx.encounterId;
    queueMicrotask(() => refreshCorpusStatus(true));
  }

  const title = document.createElement("div"); title.className = "panel-title";
  const idx = document.createElement("i"); idx.textContent = "AI";
  const titleCopy = document.createElement("div");
  const h3 = document.createElement("h3"); h3.textContent = "Encounter Intelligence Corpus";
  const sub = document.createElement("p"); sub.textContent = "Durable hosted WCL corpus · background workflow · diverse raid groups · train/holdout validation · incremental enrichment";
  titleCopy.append(h3, sub); title.append(idx, titleCopy); panel.append(title);

  const status = corpusState;
  const processed = Number(status?.processedWideCount || 0);
  const deep = Number(status?.processedDeepCount || 0);
  const pulls = Number(status?.pullCount ?? ((status?.aggregate?.killPulls || 0) + (status?.aggregate?.wipePulls || 0)));
  const deepPulls = Number(status?.deepPullCount ?? ((status?.aggregate?.deepKillPulls || 0) + (status?.aggregate?.deepWipePulls || 0)));
  const target = Number(status?.targetPulls || status?.targetReports || corpusTargetReports || 1000);
  const deepTarget = Number(status?.deepTargetPulls || status?.deepTargetReports || Math.min(1500, Math.max(200, Math.round(target * .12))));
  const discoveredSources = Number(status?.sourceStats?.total || status?.aggregate?.discoveredSourcePool || 0);
  const representedSources = Number(status?.aggregate?.independentSources || 0);
  const model = status?.model;
  const validation = Number(model?.validationScore);
  const learned = Number(model?.learnedPct);
  const modelValue = Number.isFinite(learned) && learned > 0 ? `${Math.round(learned)}% LEARNED` : (model?.status ? String(model.status).toUpperCase() : "NOT BUILT");
  const modelMeta = model ? `${String(model.status || "candidate").toUpperCase()}${model.learningGrade ? ` · ${model.learningGrade}` : ""} · ${model.acceptedMechanics || 0} accepted · ${Number.isFinite(validation) ? `${Math.round(validation)}%` : "—"} holdout` : "Generated pack replaces manual boss coding only after semantic + holdout validation";
  const grid = document.createElement("div"); grid.className = "corpus-grid";
  grid.append(
    corpusCell("WIDE CORPUS", `${corpusNumber.format(pulls)} / ${corpusNumber.format(target)}`, `${corpusNumber.format(processed)} reports · ${corpusNumber.format(representedSources)} represented sources · ${corpusNumber.format(discoveredSources)} discovered`),
    corpusCell("DEEP SAMPLE", `${corpusNumber.format(deepPulls)} / ${corpusNumber.format(deepTarget)}`, `${corpusNumber.format(deep)} reports · incomplete streams excluded per metric`),
    corpusCell("BOSS LEARNED", modelValue, modelMeta),
    corpusCell("WCL BUDGET", status?.rateLimit?.pointsRemaining != null ? corpusNumber.format(status.rateLimit.pointsRemaining) : "—", status?.rateLimit?.limitPerHour ? `of ${corpusNumber.format(status.rateLimit.limitPerHour)} points/hour remaining` : "Rate-aware automatic pause/resume")
  );
  panel.append(grid);

  if (model?.learningComponents) {
    const learning = document.createElement("div"); learning.className = "corpus-learning";
    const parts = [
      ["DATA", model.learningComponents.dataDepthPct],
      ["HOLDOUT", model.learningComponents.holdoutPct],
      ["SIGNALS", model.learningComponents.signalCoveragePct],
      ["SEMANTICS", model.learningComponents.semanticResolutionPct],
      ["DIVERSITY", model.learningComponents.diversityPct],
    ];
    for (const [label, value] of parts) {
      const cell=document.createElement("div"), l=document.createElement("span"), b=document.createElement("b"), bar=document.createElement("i"), fill=document.createElement("em");
      l.textContent=label; b.textContent=Number.isFinite(Number(value))?`${Math.round(Number(value))}%`:"—"; fill.style.width=`${Math.max(0,Math.min(100,Number(value)||0))}%`; bar.append(fill); cell.append(l,b,bar); learning.append(cell);
    }
    const rec=model.enrichmentRecommendation; if(rec){const note=document.createElement("p"); note.textContent=`NEXT: ${String(rec.mode||"review").replaceAll("-"," ").toUpperCase()} · ${rec.reason||"Review the model before enriching."}`; learning.append(note);}
    panel.append(learning);
  }

  const progress = document.createElement("div"); progress.className = "corpus-progress";
  const progressFill = document.createElement("i");
  const phaseName = String(status?.phase || "");
  const phaseProgress = phaseName === "deep" ? status?.progress?.deep : (phaseName.startsWith("discover") || phaseName === "expand-sources") ? status?.progress?.discovery : status?.progress?.wide;
  progressFill.style.width = `${Math.round(Math.max(0, Math.min(1, Number(phaseProgress || 0))) * 100)}%`;
  progress.append(progressFill); panel.append(progress);

  const msg = document.createElement("div"); msg.className = "corpus-message";
  const phase = document.createElement("b"); phase.textContent = String(status?.phase || "idle").toUpperCase();
  const message = document.createElement("span");
  if (status?.status === "rate-limited" && status?.resumeAt) {
    const countdown=corpusCountdown(status.resumeAt); message.textContent=`WCL rate budget protected · ${countdown ? `resume in ${countdown}` : "waiting for reset"}. The durable workflow will wake automatically.`;
  } else if (status?.message) message.textContent = status.message;
  else message.textContent = "Build in the background from a large public WCL cohort. You can close this tab; the durable workflow checkpoints to private persistent storage and resumes independently.";
  msg.append(phase, message); panel.append(msg);

  if (model?.status === "candidate" && model?.thresholds) {
    const gate = document.createElement("div"); gate.className = "corpus-gate";
    const checks = model.publishChecks || {};
    gate.textContent = `AUTO-PUBLISH GATE · ${model.thresholds.minWidePulls?.toLocaleString?.() || 2500}+ pulls · ${model.thresholds.minDeepPulls?.toLocaleString?.() || 300}+ deep · ${model.thresholds.minIndependentSources || 50}+ represented sources · ${model.thresholds.minValidationSources || 12}+ isolated holdout sources · ≥${Math.round(model.thresholds.minLearnedPct || 82)}% learned · ≥${Math.round((model.thresholds.minSemanticCoverage || .70) * 100)}% semantics · ≥${Math.round((model.thresholds.minSignalCoverage || .75) * 100)}% signal coverage · no critical unresolved signals${Object.values(checks).every(Boolean) ? " · PASSED" : " · NOT YET PASSED"}`;
    panel.append(gate);
  }

  const actions = document.createElement("div"); actions.className = "corpus-actions";
  const select = document.createElement("select");
  for (const n of [1000, 5000, 10000]) { const o = document.createElement("option"); o.value = String(n); o.textContent = `${corpusNumber.format(n)} target pulls`; select.append(o); }
  select.value = [1000,5000,10000].includes(corpusTargetReports) ? String(corpusTargetReports) : "1000";
  select.disabled = corpusDriving || ["running","rate-limited"].includes(status?.status);
  select.addEventListener("change", event => { corpusTargetReports = Number(event.target.value) || 1000; });
  actions.append(select);

  if (!status) {
    actions.append(corpusButton("BUILD CORPUS", async () => {
      const deepGoal = Math.min(1500, Math.max(200, Math.round(corpusTargetReports * .12)));
      await corpusRequest("start", { targetPulls: corpusTargetReports, deepTargetPulls: deepGoal });
      pollCorpus();
    }));
  } else if (["running","rate-limited"].includes(status.status)) {
    actions.append(corpusButton("PAUSE", async () => { corpusDriving = false; await corpusRequest("pause"); }));
  } else if (status.status === "paused") {
    actions.append(corpusButton("RESUME", async () => { await corpusRequest("resume"); pollCorpus(); }));
  } else if (status.status === "ready") {
    actions.append(corpusButton("RECOMPILE · 0 WCL", async () => { await corpusRequest("recompile"); }));
    const rec=model?.enrichmentRecommendation;
    const addWide=Math.max(0,Number(rec?.suggestedAdditionalWidePulls)||500), addDeep=Math.max(0,Number(rec?.suggestedAdditionalDeepPulls)||100);
    if(addWide>0||addDeep>0) actions.append(corpusButton(addWide>0?`ENRICH +${corpusNumber.format(addWide)} PULLS`:`ENRICH DEEP +${corpusNumber.format(addDeep)}`, async () => { await corpusRequest("enrich", { addPulls:addWide, addDeepPulls:addDeep }); pollCorpus(); }));
  }
  actions.append(corpusButton("REFRESH", () => refreshCorpusStatus(true), "ghost"));
